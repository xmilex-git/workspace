...
> **정정 (2026-07-28, 같은 날 후속 라운드)** — 아래 두 항목 중 첫째는 원인이
> 규명됐고 둘째는 **내 판단 오류였다.** 확정판은
> `docs/report-q1-abcd-counters-round2-20260728.md` B절이다.
>
> * "부착 방식에 따라 15배" — 미해결이 아니다. 같은 질의 창에서 SUT 부착과 시스템
>   와이드를 **동시에** 재보니 PG LLC-load-misses는 SUT 부착 6.07 M, 시스템 와이드
>   100.0 M으로 **오염률 93.9 %**다. 즉 넓은 부착·시스템 와이드에서 나온 큰 값은
>   SUT 밖 배경 활동을 함께 센 것이고, 좁은 부착값(5회 5.07~6.33 M)이 SUT 값이다.
> * "시스템 와이드는 신뢰할 수 없다" — **틀렸다.** 코어 14에 고정한 통제 busy 루프를
>   per-process와 `-a -C 14`로 재면 instructions 14,030,607,865 vs 14,013,490,579로
>   **0.12 % 일치**하고, `-a -C 0-15`의 cycles를 CPU-초로 환산한 값(22.94)이
>   `/proc/stat` 실측(21.63)과 6 % 내로 맞는다. "idle 9초에 469 G"는 perf 결함이
>   아니라 그 순간 배경 부하가 실제로 무거웠던 것이다(배경 부하 1.9~52 G
>   instructions/s로 변동). `perf -a -C`는 쓸 수 있고, **다만 SUT 밖 활동을 함께
>   세므로 카운터별 오염률을 반드시 병기해야 한다.**
