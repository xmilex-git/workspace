...
- 공식 Notion 커넥터가 미노출·미연결이어도 측정 세션 운영은 중단하지 않는다. 해당 반영만
  `deferred Notion backfill`로 남기고, heartbeat에 기록된 정확한 현재 QNN/세션 이름을
  전환 중 임시 운영 상태로 사용한다. 커넥터가 복구되면 누락된 완료·세션 전이를 순서대로
  반영하고 재조회 검증한다.
...
- 현재 질의의 필수 단계와 idle을 확인한 뒤 Notion 최종 반영을 시도한다. 공식 커넥터가
  호출 가능하면 재조회 검증한 뒤, 호출 불가능하면 해당 반영을 `deferred Notion backfill`로
  기록한 뒤 현재 GJC 세션을 먼저 `gjc session remove <현재세션>`으로 닫는다.
...
- 종료 검증 후에만 `~/dev/workspace`에서 `gjc session create -j`로 다음 세션 하나를 생성한다.
  반환된 정확한 세션 이름은 즉시 heartbeat 현재 상태에 반영한다. 공식 Notion 커넥터가
  호출 가능하면 해당 Notion 행에 기록하고 재조회 검증한 다음 프롬프트를 전송하며, 호출
  불가능하면 Notion 반영만 지연하고 같은 세션에 프롬프트를 전송한다.
...
다음 조건에서는 원격 세션 입력과 Notion 변경을 중단한다.
...
공식 Notion 커넥터의 미노출·미연결 자체는 정지 조건이 아니며, Notion 반영만 지연한다.
