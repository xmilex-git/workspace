...
> **이 문서는 `tpch-sspq/impl/HANDOFF-FINAL.md`로 대체되었다.** 다음 세션에 넣을 프롬프트와
> 사용자 선행 결정 항목은 그쪽이 정본이다. 이 파일은 이력 기록으로 남긴다 — 캠페인의 반복
> 실패 모드가 요약 문서 간 드리프트였으므로, 링크 없이 공존하는 인수 문서를 두지 않는다.
...
### 캠페인 산출물 자체는 게이트를 통과했다

경계 코호트 generation 13에서 `productStatus: CLEAR` / `codeStatus: CLEAR`를 받았고,
그 이후 `fresh-baseline.json` / `restart-variance-calibration.json` /
`priority-ranking.json` / `benefit-inputs.json` / `harness/`는 **수치가 한 번도 바뀌지
않았다**(각 세대마다 수치 leaf 불변을 확인). 즉 베이스라인·보정·랭킹·큐·민감도는
심사를 통과한 상태다.

### 통과하지 못한 것: 요약 문서 3건 + 산출물 생산자 결함 1건

generation 13~15가 잡은 것은 `PHASE-RECORD-2026-08-05.md`와
`HANDOFF-phase2-and-beyond.md`, 그리고 그 검증용으로 만든 `doc-claim-audit.md`의 서술
결함이다. **그러나 generation 16은 서술이 아니라 산출물 생산자의 결함을 잡았다** — 아래
표의 마지막 두 행이 그것이고, 이 문서의 이전 판이 "전부 서술 결함"이라고 적은 것은
**틀렸다**. 터미널 critic이 그 과대 주장을 지적했고 맞다.

| 세대 | 잡힌 결함 | 성격 |
|---|---|---|
| 13 | 후보 큐를 23개로 적고 `IMP-017` 포함 | 구조 오기. 다음 세션이 diagnostic 후보를 Phase 2 항목으로 착수할 뻔했다 |
| 14 | Q01 median `31.65` (실제 `31.64`) | 반올림 오기 |
| 15 | Q09 무효 사유 코드와 `external_max` 범위 | 두 층(블록/시도)을 혼동하고 네 값 중 하나를 빠뜨렸다 |
| 16 | `doc-claim-audit.md`가 파일명을 **날조**하고, 정규식이 한글 경계 때문에 과소 계수하고, 라운딩 결속이 무관한 필드를 잡았다 | 검증 문서가 검증을 통과하지 못했다 |
| 16 | **`aggregate_baseline._load_attempts()`가 폐기된 pre-AMEND-G 런의 로그를 읽고 있었다** | **서술이 아니라 산출물 결함.** baseline이 다른 런의 시도 무효화 16건(그중 14건은 AMEND-G에서 불가능한 `WARM_NOT_CONVERGED`)을 실었다 |
| 16 | 나는 그 틀린 숫자(`Q09.attempts_invalidated: 0`)에 카운터 의미론 설명을 붙여 합리화했다 | 원인 규명이 아니라 결과 무마. 실제 값은 **7** |
...
한계(인과 서술·판단·누락은 덮지 못함)를 명시한다. 그 문서 맨 앞에 v1·v2가 틀렸다는 사실을
남겨 뒀다.

**다음 세션에 주는 실질적 조언**: 이 두 문서의 서술을 근거로 쓰지 말고 **산출물 JSON을
직접 읽어라.** 문서는 지도이고 산출물이 영토다. 네 세대에 걸쳐 지도만 틀렸다.

### 형식적으로 게이트는 아직 닫히지 않았다
...
- `harness/`에 상주 단위 시험이 없다. 생성기 계약은 코호트 QA 레인이 매 세대 재작성해
  검증했을 뿐이다. Phase 2 전에 최소한 `score_with()`의 모집단·정렬 불변식과
  `state_labels`의 양방향 속성은 상주 시험으로 고정할 가치가 있다.

**판단**: 게이트를 닫는 것과 Phase 2를 시작하는 것은 별개다. Phase 2는 어차피 §6-d-1
인자 결정과 랭킹 승인을 기다린다(§1의 승인 대기 8건). 남은 게이트 작업은 요약 문서의
정확성에 관한 것이고 측정 결과를 바꾸지 않는다.
