...
| 5 | Stress/diagnostic | Concurrency stress: 4 concurrent csql × 3 iterations of Q10 on the patched binary — 0 failures, 12/12 byte-identical to the single-run reference. Sanitizer/assertion build not run; disposition: executor-only parallel-decision change; the one concurrency hazard found (latch deadlock) is exercised directly by the TC and stress |
...
- MDE: the pinned §6-d-1 **corrected MDE does not exist** — Phase 1A never ran. Substitute, explicitly labelled: `MDE_proxy = max(1%, 2 × CV_B) = 1.42%`. Because these blocks ARE restart-regime and same-query, this proxy measures exactly the quantity §6-d-1's inflation exists to reconstruct; it differs from the pinned procedure only in source (this A/B's own B blocks instead of a Phase 1A baseline). Point improvement 9.92% ≈ 7 × MDE_proxy.
...
**accepted** — deciding criteria (§7-a): (1) all §6-b checks pass; (2) paired bootstrap 95% CI [0.8991, 0.9223] entirely below 1.0; (3) point improvement 9.92% ≥ MDE — evaluated against the labelled restart-regime MDE_proxy 1.42% because the pinned corrected MDE cannot exist without Phase 1A (disclosed, not hidden); (4) the named metric signature moved as predicted (GROUPBY gains `parallel workers ≥ 2` sub-line; sort-worker ioread ~0; GROUPBY ioread -63%); (5) no non-target regression above 3% in the available control set. Caveats carried with the verdict: the absolute Q10 card gate (≤5.70 s) is unmet for regime reasons (§5), TWU not measured, controls at 1-block resolution, §7-e magnitude divergence documented (§8).
