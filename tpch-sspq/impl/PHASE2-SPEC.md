...
### 추가 사용자 결정 (2026-08-06, 3차): 판정 가능성 문턱 — 5초·10%

- "수행시간이 5초 미만인 질의를 개선하는 IMP는 이번 phase2에서 다루지 말자. 진짜 성능이
  개선되었는지 명확히 판단해내기 어려워서. 수행시간이 5초 이상이고 실질개선이 10% 이상인
  IMP 건들만 진행한다." + "그래도 IMP-005는 먼저 하긴 해야 된다."
- 규칙: 후보가 Phase 2 실행 대상이 되려면 **표적 질의의 베이스라인 median ≥ 5.0 s**이고
  그 질의 wall의 **정량화된 기대개선 ≥ 10%**여야 한다. 둘 중 하나라도 못 넘으면 구현·측정
  없이 종결한다. 근거 산출물: `impl/queue-filter-20260806-5s-10pct.json`
  (medians = fresh-baseline.json, 기대개선 = benefit-inputs.json / priority-ranking.json
  per_query_terms — 손 전사 없이 생성).
- 예외 1건: **IMP-005**는 사용자 지시로 그대로 먼저 실행한다. 성능 후보가 아니라 IMP-009의
  trace·병렬도 판독을 신뢰 가능하게 만드는 enabler이며, 설계상 런타임 효과가 0이다.
- 재판정 결과 — **실행 큐 2개**: IMP-005 (enabler 예외) → **IMP-009** (Q05 10.34 s,
  기대개선 28.4% = 보수 기대절감 2.933 s / 10.336 s, 근거등급 upper_bound).
- 종결(구현 없음) 14개와 사유:
  - 표적이 5 s 미만: IMP-018(Q11 3.23 s, 58.4%), IMP-029(Q22 1.12 s, 14.6%),
    IMP-023(Q16 2.92 s, 9.1%), IMP-030(Q22 2.3%), IMP-031(Q22 2.1%).
    → 개선폭이 커도 질의가 작아 판정 불가라는 이 결정의 정확한 표적.
  - ≥5 s 표적이 있으나 그 질의에 대한 정량 근거가 없음(NO_NUMERIC_BASIS / wall 주장 불가):
    IMP-013(Q08 10.6%만 정량, Q09·Q13·Q19·Q21 미정량), IMP-012(Q08 12.2%만 정량,
    Q19 미정량), IMP-016(Q18 attribution 1.32%), IMP-004(Q13·Q16 미정량, Q02는 0.36 s),
    IMP-021(Q13 근거는 격리 마이크로벤치 1.5617×, Q13 wall 귀속 없음), IMP-010(Q06 3.85 s
    페이지리드만 정량), IMP-008(Q04 18.1% CPU만, Q19·Q21 미정량), IMP-024(Q10 미정량),
    IMP-006(Q03 CPU 상한만, Q10·Q21 미정량).
    → Phase 1이 이미 정량화를 시도해 실패한 지점이므로, 이 규칙 아래서는 캠페인이 판정할
    수 없다. 구현 전 별도 정량화 작업을 사용자가 승인하면 재진입 가능(§11-a 항목).
- 이 결정은 §6-d-1 결합규칙 (c)를 대체하지 않는다. (c)는 통계 판정의 해상도, 이 규칙은
  착수 자격이다. 두 규칙이 함께 적용된다.
