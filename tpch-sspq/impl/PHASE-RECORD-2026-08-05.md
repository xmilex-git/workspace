...
> **이 문서는 진행 기록이다.** 다음 세션에 넣을 프롬프트와 사용자 선행 결정 항목은
> `tpch-sspq/impl/HANDOFF-FINAL.md`가 정본이며, 인수는 그쪽을 먼저 읽어라. 이 파일은
> Notion 미러(`3b1f947f-1be1-81d9-b7d4-fecf8bc04fbe`)의 Git 정본으로서 무엇을 어떻게
> 측정했는지를 남긴다. 두 문서가 수치에서 어긋나면 산출물 JSON이 정본이다.
...
- 모형이 직접 반증된다. **Q03(4.539 s, 인자 1.4516)과 Q06(3.846 s, 인자 6.4235)** 은 벽시계가
...
13세대 이후로는 심사 대상이 코드에서 **이 문서들**로 옮겨갔고, 세 세대가 연속으로 내가 손으로
옮겨 적은 주장의 오류를 하나씩 잡았다 — 후보 큐 23개 오기(gen13), Q01 median 31.65(gen14),
Q09 무효 사유 코드와 external_max 범위(gen15). 16세대는 그 대응으로 만든 검증 문서
`doc-claim-audit.md` 자체에서 날조된 파일명과 과소 계수하는 정규식을 잡아냈다. 상세는
그 문서 맨 앞에 적었다.
