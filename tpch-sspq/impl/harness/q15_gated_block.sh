#!/usr/bin/env bash
# TPCH-SSPQ implementation campaign (tpch-sspq-impl-r1-20260803) — Q15 gated
# measurement block. Campaign-local adaptation per IMPL-SSOT section 8-b;
# CAMPAIGN, RAW_ROOT and THRESHOLD come from campaign_env.sh.
#
...
# THRESHOLD is sourced from campaign_env.sh (section 8-b).
...
# driver, so this script must not remove the query-level warm artifact it did not
# produce and will not reproduce. Q15's warm unit is the logical session (create
# view / select / drop view), not the statement — that is unchanged.
...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  if [ "$SKIP_WARM" = 1 ]; then
    echo "  warm SKIPPED (AMEND-G fast regime; WARM proved once per query by the driver)"
  else
