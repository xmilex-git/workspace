#!/usr/bin/env bash
# TPCH-SSPQ implementation campaign (tpch-sspq-impl-r1-20260803) — gated
# measurement block runner. Campaign-local adaptation of the inherited
# tpch-sspq/harness/measure_block.sh, per IMPL-SSOT section 8-b: CAMPAIGN,
# RAW_ROOT and THRESHOLD now come from campaign_env.sh (which derives them from
# campaign_config.py) instead of the previous campaign's literals.
#
# IMPL-SSOT section 3-a: "If external CPU on the SUT set is above 6.0 core-seconds per second
...
# THRESHOLD comes from campaign_env.sh below, sourced from the pinned section
# 3-a value. It is deliberately NOT a literal here (section 8-b).
...
# AMEND-G: in the fast Phase 1A regime WARM is established ONCE per query by the
# driver, not once per block, so this script must not remove the query-level warm
# artifact it did not produce and will not reproduce.
SKIP_WARM="${TPCH_SSPQ_SKIP_WARM:-0}"
rm -f "$W/${QNN}-${LABEL}-headline.json" \
      "$W/${QNN}-${LABEL}-bgload.json" "$W/sink/${QNN}-${LABEL}-headline.out"
[ "$SKIP_WARM" = 1 ] || rm -f "$W/${QNN}-${LABEL}-warm.json"
...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  if [ "$SKIP_WARM" = 1 ]; then
    # AMEND-G fast regime: the server has not been restarted since the driver
    # proved WARM for this query, so the engine cannot have left the state that
    # proof established. Re-proving it per block is exactly the ~30
    # execution-equivalents per block that AMEND-G exists to remove. The
    # query-level warm.json the driver wrote stays the block's warm evidence.
    echo "  warm_establish SKIPPED (AMEND-G fast regime; WARM proved once per query by the driver)"
  else
...
json.dump({"campaign_id": "tpch-sspq-impl-r1-20260803", "qnn": qnn, "engine": eng,
           ...
           "note": f"IMPL-SSOT section 3-a: external SUT-set load crossed {thr} core-s/s during the block"},
