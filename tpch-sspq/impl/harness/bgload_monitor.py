...
"""
TPCH-SSPQ implementation campaign (tpch-sspq-impl-r1-20260803) — external SUT-set load monitor.
Campaign-local adaptation (IMPL-SSOT section 8-b); the 6.0 core-s/s gate is section 3-a / AMEND-D.
...
SUT_CPUS = cfg.SUT_CPU_SET
COLLECTOR_CPUS = cfg.COLLECTOR_CPU_SET
...
def main():
    ...
    # AMEND-D pins 6.0 core-s/s in section 3-a. The default comes from the
    # pinned config, never from an inherited literal.
    threshold = (float(sys.argv[3]) if len(sys.argv) > 3
                 else cfg.EXTERNAL_LOAD_THRESHOLD)
