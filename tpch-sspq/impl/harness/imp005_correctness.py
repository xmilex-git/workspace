...
"""
IMP-005 section 6-b correctness gate (adapted from imp015_correctness.py) — base-versus-patch on the SAME engine.
...
Covers 6-b mandatory checks 2, 3 and 4 in one sweep:
  - target queries Q05 / Q07 / Q08 / Q09 / Q21,
  - every q_relations query (Q03-Q13, Q15, Q17, Q19, Q21),
  - the full Q01-Q22 result smoke,
  - the Q15 view protocol: proved absent before, dropped and re-proved after.

Usage:
  imp015_correctness.py run --variant {base,IMP-005} --outdir DIR
...
def die(msg):
    print(f"IMP005-CORRECTNESS FATAL: {msg}", file=sys.stderr)
    sys.exit(1)
...
def cmd_run(args):
    ...
    print(f"IMP005-CORRECTNESS run {args.variant}: PASS ({outdir})")
...
def cmd_compare(args):
    ...
    print("IMP005-CORRECTNESS compare: PASS (all queries EXACTLY equal)")
