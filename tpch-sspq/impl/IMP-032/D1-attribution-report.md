...
**계수 방법 주의**: 위 표의 심볼 카운트는 콜체인 라인에 대한 **부분문자열** 매칭이다. `sort_listfile`처럼 다른 심볼의 접두사인 이름은 파생 심볼까지 함께 집계되므로(480 vs 정확 토큰 459), 이 항목은 존재/부재 판정에만 쓰고 정량 비교에는 쓰지 않는다. 판정에 사용한 심볼 중 **두 개는 실제로 접두사 충돌이 있다** — `nm -C --defined-only` 로 확인: `sort_merge_nruns` ⊂ `sort_merge_nruns_queue_cb`, `sort_exphase_merge` ⊂ `sort_exphase_merge_elim_dup` (독립 red-team gen2 검증, `closeout/redteam/redteam-report-gen2.json`). 나머지 네 개(`sort_put_result_from_tmpfile`, `sort_end_parallelism`, `qexec_gby_put_next`, `qexec_hash_gby_put_next`)는 충돌이 없다. 충돌하는 두 심볼에 대해서는 red-team이 두 프로파일 전 표본을 **표본별로 정확토큰 계수와 부분문자열 계수를 대조**해 이 보고서의 수치에 왜곡이 없음을 확인했다. 즉 근거는 '충돌이 존재하지 않는다'가 아니라 '충돌이 이 수치들을 흔들지 않음을 표본별로 검증했다'다. 또한 독립 파서 재계수(`closeout/redteam/recount.py`)로 Q15 5,088/0/0 및 Q18 10,684/716/103/1,203/1,700/688/0 전 항목 일치를 확인했다.
