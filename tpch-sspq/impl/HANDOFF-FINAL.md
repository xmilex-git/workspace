...
- 모형 직접 반증 — Q03(4.540 s, 1.4516)과 Q06(3.846 s, 6.4235)은 벽시계 1.18배 차이인데 인자
  4.43배 차이
...
| (d) **보정 블록 추가 수집** | Q01~Q06 fast 레짐 블록 수를 늘려 3-pair 분모 불확실성을 줄인 뒤 다시 판단 | 미정 — 측정으로 결정 |

**내 권고: (d).** 불안정의 원인이 분모의 표본 부족으로 규명됐으므로, 모형을 고르는 것보다
측정을 늘리는 것이 정직하다. (a)~(c)는 어느 것이든 6점이 지지하지 않는 가정을 얹는다.
급하면 (c)가 안전측이다 — 절대 false accept를 만들지 않는다(대신 진짜 개선을 놓친다).
