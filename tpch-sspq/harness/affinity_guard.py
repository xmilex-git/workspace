...
"""TPCH-SSPQ FK campaign (tpch-sspq-fk-r1-20260730) — all-TID cpuset guard.
...
PG_BIN = "/home/cubrid/pg/pg20devel-5713b437/bin/postgres"
PGDATA = "/home/cubrid/pg/pgdata-tpch-sspq"
CUBRID_DB = os.environ.get("CUBRID_DB", "tpch_sf10_q1")
...
def reapply():
    ...
    for (label, pid, tid), (is_off, mask, cm, _) in sorted(st.items()):
        if not is_off:
            continue
        r = subprocess.run(["taskset", "-p", "-c", "0-15", str(tid)],
...
def main():
    ...
    result = {
        ...
        "stage": "all-TID cpuset guard (SSOT s9 / s24)",
        "sut_cpus": sorted(SUT),
        "interval_s": interval,
        "samples": samples,
