...
Q8과 같았다. **CUBRID는 세 쿼리 모두 무감**하다. 두 하위 레짐의 값을 같은 표에 합치지 않으며,
모든 측정 표에 하위 레짐 필드를 적고 **PG 표에는 `shared hit`/`read` 분할을 같이 적는다**. (ADR 0016)
...
Q9 실측 **instructions 4.233x / cycles 3.022x / wall 2.741x** (IPC CUBRID 1.929 vs PG 1.378
= 1.40x), Q8 실측 **instructions 5.796x / cycles 3.734x / wall 3.960x** (IPC 2.132 vs 1.374
= **1.55x**). Q1은 IPC가 사실상 같아(2.339 vs 2.357) 이 문제가 드러나지 않았다.
**`instructions` 비는 시간 격차의 상한**이고 **`cycles` 비가 시간 비에 가깝다.** 주 표는
`instructions`로 두되 `cycles` 표를 항상 같이 내고, **cycles에서 버킷 순위가 뒤집히는
지점을 명시한다**(Q9: 버퍼 고정·래치 instr 3위 13.3 % → cycles 1위 27.8 %; 집계·해시는 기여
부호가 +2.2 % → −4.7 %로 역전 — PG `ExecParallelScanHashBucket`이 IPC 0.27로
PG cycles의 21.2 %를 쥔다. Q8: 같은 두 역전이 재현되고 버퍼 고정·래치 instr 5위 8.4 % →
cycles 2위 19.8 %, 집계·해시 −3.4 % → −10.0 %). **`cycles:u`가 wall보다 커지는 경우도
있다** — Q8 형상 일치 쌍에서 cycles 4.910x vs wall 3.964x이고, 차는 이용률 비(1.074x)와
커널 시간 비중 차(PG sys 23.2 % vs CUBRID 11.9 %)로 2 % 내에 닫힌다.
(Q9 규명 `docs/report-q9-gap-20260729.md`, Q8 규명 `docs/report-q8-gap-20260729.md`)
_Avoid_: 프로파일 배수, 명령 수 배수 (이벤트가 불명확)
...
실측 배수와 맞는지 **검산을 반드시 낸다.** 실측 4건 — Q21 `14.319x = 4.436x × 3.228x`
...
**실행 단위 축이 항상 닫히는 것은 아니다** — Q8은 **1.129x(격차의 12.9 %)**로 열려 있고,
원인은 gather 위 중간 리스트 스캔이 `parallel_scan_page_threshold=2048`에 걸려 직렬로
떨어지는 것이다(wall의 15.1 %가 실행 단위 1).
...
**+6.56 %**(2.953 s/런), **Q8 SUT CPU의 +20.8 %**(3.196 s/런 — 현재 최대). 세 열을
