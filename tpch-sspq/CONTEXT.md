...
분할, CUBRID trace `fetch`/`ioread`). **PG의 민감도는 쿼리 조건부다** — 대상 쿼리 워킹셋이
`shared_buffers`에 들어갈 때만 민감하다(Q21 12.72x ↔ 14.32x, 총 버퍼 접근은 1블록 차이로
동일). **들어가지 않으면 무감하다**: Q9 PG는 미스를 −39.9 % 줄여도 wall이 **+2.70 %**였고,
그 `read` 전량이 `Parallel Seq Scan on lineitem`(힙 1,125,128 블록 = 8.6 GB > 8 GB)에
있었다. **CUBRID는 두 쿼리 모두 무감**하다. 두 하위 레짐의 값을 같은 표에 합치지 않으며,
모든 측정 표에 하위 레짐 필드를 적는다. (ADR 0016)
...
= 1.40x). Q1은 IPC가 사실상 같아(2.339 vs 2.357) 이 문제가 드러나지 않았다.
...
PG cycles의 21.2 %를 쥔다). (Q9 규명, `docs/report-q9-gap-20260729.md`)
