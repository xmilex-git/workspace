...
**rev 2 개정 (ADR 0020 §4)** — 이 항의 용어는 **`configured-cap parity`**로 대체됐다(아래 항 참조).
`parallelism`은 **노드 단위 상한**이고 전역 예산은 `max_parallel_workers`(CUBRID 100 ↔ PG 8)다.
"양쪽 6단위"라는 표현은 폐기되며, 표에는 `실행 단위 수` 한 행이 아니라 5개 축을 따로 적는다.
_Avoid_: DOP, worker 수, 병렬도, 실행 단위 파리티, 양쪽 6단위 (실행 단위와 다를 수 있다)

**Natural / unit-parity-controlled (2트랙)**:
병렬 지표를 낼 때 **(1) 자연 구성값** — 양쪽을 같은 설정 숫자로 놓은 값(양쪽 "DOP 6",
PG는 실제로 7단위) — 과 **(2) `configured-cap parity` 통제값** — 양쪽 **노드 단위 cap**을 맞춘 값 — 을
나란히 낸다. **헤드라인 배수는 (2)를 쓴다.** natural-plan / plan-family-controlled
2트랙 규칙과 같은 구조이며, 두 트랙을 섞은 단일 숫자는 내지 않는다. (ADR 0014+0020 §4)
_Avoid_: 배수, 격차 (어느 트랙인지 불명확), 단위 파리티 통제값 (rev 1 용어)
...
9.0 GB written)에 있기 때문이다. **Q15는 PG가 stream ↔ single-query-repeat에서 −4.90 % 움직인다**(2.023x → 2.179x)— CUBRID는 +2.49 %다. **CUBRID는 여섯 쿼리 모두 무감**하다. 두 하위 레짐의 값을 같은
표에 합치지 않으며, 모든 측정 표에 하위 레짐 필드를 적고 **PG 표에는 `shared hit`/`read` 분할을
같이 적는다**. (ADR 0016)
**Q3에서 판정 기준이 다시 좁혀졌다 (ADR 0020 §5)** — "잔존율이 움직이는가"는 **필요조건이지
충분조건이 아니다.** Q3 PG는 lineitem 잔존율이 **30.2 % ↔ 66.6 %(+36.4 %p, Q8의 3.6배)**로
크게 움직이고 `hit+read` 1,422,457이 **완전 동일**한데 `Execution Time`은 **−1.86 %**뿐이다
(미스 단가 **0.175 µs** — Q21 0.81 / Q8 2.62보다 자릿수가 작다. 미스가 전부
`Parallel Seq Scan on lineitem`의 순차 read이고 `io_method=worker` 프리페치가 스캔의 CPU 시간
1,488 ms/worker 뒤에 숨는다). 따라서 판정 기준은 **"미스 감소량 × 미스 단가가 wall에서
관측되는가"**이고 **미스 단가를 항상 같이 적는다.**
...
**Q15: 버퍼 고정·래치 역전이 네 번째로 재현**(instr 5위 7.5 % → **cycles 2위 26.1 %**)되고 **스캔·레코드 디코드가 부호까지 반전**한다(instr +17.01 G/1.28x → **cycles −0.91 G/0.97x 동률**). **Q18: 같은 역전이 세 번째로 재현**(버퍼 고정·래치 instr 3위 7.9 % → **cycles 2위 14.0 %**,
...
**Q3: 역전이 다섯 번째로 재현되되 형태가 다르다** — 순위는 2위로 같고 **절대 델타 자체가
instr +28.31 G → cycles +33.57 G로 커져** 1위 B-tree와의 격차가 47.9 %p → **1.8 %p로 붕괴**한다
(버퍼 버킷 IPC **0.83**, B-tree 버킷 IPC 2.06). 스캔·디코드는 Q15와 같은 방향으로 움직이되
부호는 유지한다(instr 0.82x/−10.1 % → cycles **0.95x**/−1.6 %). **`cycles:u`가 wall보다 큰 두 번째
사례**이고(2.4397x vs 2.2771x) 차는 유효 클럭 비(`cycles:u`/task-clock = 2.476/2.289 GHz = 1.0817)와
`time-weighted active units` 비(1.0098)로 **0.04 %** 내에 닫힌다.
**모든 배수·기여도에 이벤트 단위를 붙이고 혼용하지 않는다**(ADR 0020 §5) —
`[wall]`/`[질의실행분 CPU-초]`/`[cycles]`/`[instructions]`/`[time-weighted active units]`.
**Q8 분해는 `0.999x × 0.9314x × 4.2559x` 또는 `3.508x × 1.129x`로만 적는다.**
**채증 함정** — `perf record -a -C`(perf 4.18)는 **레코드 시작 전 존재하던 sibling 스레드**의 map
group을 프로세스에 연결하지 못해 그 스레드 샘플이 전부 `[unknown]`이 된다. CUBRID는
`parallel-query` 워커를 풀로 재사용하므로 Q3 정본 프로파일이 **99.95 % unknown**이었다(PG는 워커가
매 런 fork되어 무영향). 보정 도구 `q3/scratch/resolve.py`(MMAP2 + `readelf -sW` + `c++filt`
오프라인 심볼라이즈), perf 해석분과의 일치율을 항상 검산한다(Q3 instructions 95.68 % / cycles 96.42 %).
...
행수로 채증한 뒤** 같은 엔진의 두 값을 비교한다. **실행 단위 축**은
`time-weighted active units 비(PG/CUBRID)`로 잰다(ADR 0018+0020 §4). **행당 실행 비용 축**은
형상이 일치한 상태의 **질의실행분 CPU-초 비**다. 세 축의 곱이
실측 배수와 맞는지 **검산을 반드시 낸다.** 실측 6건 — Q21 `14.319x = 4.436x × 3.228x`
(실행 단위 0.2 %), Q9 `2.6944x = 0.9844x × 2.7371x`(검산 오차 0.00 %, 실행 단위 0.23 %),
Q8 `3.9599x = 0.9990x × 0.9314x × 4.2559x`(검산 오차 0.01 %; **이 표기 또는 `3.508x × 1.129x`만 허용**),
**Q18 `1.2468x = 0.6643x × 1.5409x × 1.2180x`(검산 오차 0.014 %)**,
**Q15 `2.1650x = 1.0000x × 1.2620x × 1.7155x`(검산 오차 0.00 %)**,
**Q3 `2.2573x [wall] = 1.2465x [wall] × 0.7986x [units 비] × 2.2676x [CPU-초 비]`(검산 오차 0.00 %)**.
**플랜 축이 CPU를 바꾸지 않을 수 있다 (Q3, 신규)** — CUBRID의 질의실행분 CPU-초는 idx-NL 47.30 s ↔
`USE_HASH` 47.60 s로 **0.6 % 동률**이고, 플랜 축 1.2465x는 전부 `time-weighted active units`
5.61 → **7.05**(노드 cap 6의 117.5 %)에서 나온다. 노드 cap 6으로 정규화하면 두 형상이 동률
(7.88 s ↔ 7.93 s)이다. 따라서 **플랜 축을 인용할 때 그 축이 CPU를 바꾸는지 단위를 바꾸는지 명시한다.**
**실행 단위 축이 1보다 작을 수 있다 (Q3, 신규)** — A/B 변형이 `configured-cap 초과`이면 그 축은
CUBRID에 유리해진다(0.7986x). 정본 형상끼리는 0.9996x로 닫힌다.
**형상 일치 배수가 둘일 수 있다 (Q3, 신규)** — PG 형상 **1.8109x** ↔ CUBRID 형상 **1.1826x**.
둘 다 적고, 어느 형상에서 잰 값인지 없이 인용하지 않는다.
**플랜 축이 정확히 1이고 A/B 대상이 없는 경우도 있다** — Q15는 양쪽 플랜이 **네이티브로 형상 일치**(노드 계열 동일 + 노드별 실제 행수 일치: 스캔 통과 2,265,714 / 그룹 100,000 / 최종 1)라서 강제할 형상 차가 없다. 그 경우 **플랜 축 = 1.000을 형상 일치 채증으로 선언하고, 3축은 2축으로 축약**한다(`wall = 질의실행분 CPU 비 × 이용률 비`). 대신 **양쪽에 같은 재작성을 걸어 두 번째 형상 일치 쌍을 만들어 행당 축을 교차 확인한다**(Q15: 단일 평가 CTE 쌍에서 행당 1.6340x ↔ 정본 1.7155x).
...
떨어지는 것이다(wall의 15.1 %가 실행 단위 1). **Q15는 양쪽 동시 붕괴의 두 번째 사례이고 이 축이 격차의 38.6 %(2.227 s)를 쥔다(1.2620x)** — 이용률 CUBRID **59.3 %**(wall의 50.1 %가 leader 단독 GROUP BY, 뿌리는 Q18과 같은 `external_sort.c:5232`) / PG **74.8 %**(손실 전량이 `InitPlan` 3워커 = 클러스터 상한 `max_parallel_workers=8` − 외곽 gather 5, `bgworker.c:1105`). **PG 클러스터 기본값은 유지하고 이 손실을 축으로만 분해 기록하며, 배수 인용 시 `PG InitPlan 3워커(상한 8) 포함`을 명기한다.** 단일 평가 A/B가 확증한다 — PG 74.8 → **86.7 %** 복귀, CUBRID 59.0 %로 **불변**(구조적).
**Q18은 이 축이 처음으로 지배한다(1.541x)** —
...
Q18 SUT CPU의 **+3.4 %**(2.960 s/런 — 절대량은 Q9·Q8과 비슷한데 SUT가 커서 비중이 작다),
Q15 SUT CPU의 **+9.9 %**(2.207 s/런), **Q3 SUT CPU의 +2.4~11.1 %**(0.990~2.377 s/런 — 같은 쿼리에서
`io worker` pid 개수가 런 중 **4 → 8개로 동적 증가**하므로 범위로 적는다). 세 열을
합산한 단일 숫자는 내지 않는다. (ADR 0017)
_Avoid_: PG SUT CPU (io worker 포함 여부가 불명확)

**PG 귀속 검증 = `perf stat -p <postmaster>` + 새 psql 세션 (Q15 신규)**:
postmaster에 부착하고 그 뒤 새 psql 세션을 열면 **leader가 런 중 fork되어 상속**되므로 **leader + parallel worker = PG SUT를 정확히** 센다(런 이전에 이미 존재하던 io worker·bgwriter·checkpointer는 상속 대상이 아니라 자동 제외된다). CUBRID의 `perf stat -p <cub_server>`와 대칭이다. 실측 귀속 instructions **100.10 %** / cycles 99.06 %. **`-a -C 0-15` + idle 창 감산법을 대체한다** — Q15 회차의 idle 6 s 창이 배경 부하 **75.12 G instructions**를 담아 감산이 성립하지 않았다(배경 부하 실측 범위 1.9~52 G instructions/s). Q8·Q18의 감산법 수치는 이 방법으로 재검증할 대상이다.
_Avoid_: idle 감산 (배경 부하가 크면 성립하지 않는다)
...
Q15 실측: 정렬 3.5 % / 임시파일·스필 3.9 %가 전체 비중인데 **CUBRID leader만 보면 21.4 % / 22.5 %**이고, 술어 평가는 전체 20.1 %인데 **worker 24.1 % / leader 0 %**다(세 번째 재현). **신규 조항 — 직렬 구간은 클럭 이득을 받는다**: Q15 역산 클럭이 CUBRID leader **2.81 GHz** > worker 2.21 GHz다(직렬 구간엔 코어 1개만 바빠 turbo가 더 붙는다). **직렬 구간 병렬화의 상한을 계산할 때 클럭 하락을 반드시 반영한다** — 반영하지 않으면 상한이 과대평가된다(Q15 후보 A: 1.26x → 실제 1.31x). 도구 `q18/scratch/{actors.sh,actor-buckets.sh}`, `q15/scratch/actor-classify.py`. (ADR 0019 §4)
**Q3에서 조항이 좁혀졌다 (ADR 0020 §5)** — 클럭 이득은 **직렬 구간이 wall의 20 % 이상일 때만**
가정한다. Q3의 직렬 구간은 0.50 s(wall의 5.9 %)로 짧고 역산 클럭이 leader **2.378 GHz** <
worker **2.513 GHz**로 **반대 방향**이다(병렬 구간의 열 상태를 물려받는다). 20 % 미만이면 역산 클럭을
그대로 적고 이득을 가정하지 않는다. 또 **serial tail이 wall의 5 %를 넘으면 행위자별 분해를 낸다**
(20 % 문턱은 "필수" 기준, 5 %는 "산출" 기준). Q3 실측: CUBRID leader가 전체 cycles의 **1.0 %**뿐이고
IPC 2.18(worker 1.544)로 성질이 다르며, GROUP BY 정렬 병합(`qfile_compare_partial_sort_record` 8.2 %,
`sort_run_merge` 4.1 %)이 leader의 내용이다 — 그룹이 114,003개라 싸다.
PG는 leader/worker 버킷 구조가 사실상 동일하다(스캔 48.5 %/49.2 %, 집계·해시 21.2 %/21.9 %).
도구 `q3/scratch/actors.py`(perf 해석 실패 보정 포함).
_Avoid_: 프로파일 버킷 (행위자 구분이 없으면 직렬 비용이 안 보인다)

**`contract_revision` (계약 리비전)**:
측정 계약의 판번호다. **rev 1** = ADR 0001~0019까지의 계약(`G2` 명칭, "6 실행 단위 파리티" 표현,
연결 모드 미기록, censoring 미분리). **rev 2** = ADR 0020의 원칙 v2. 모든 보고서와 모든 run
`meta.json`에 이 필드를 적는다. rev 1 결과는 재측정하지 않고 재분류·재명명만 하며, rev 2 용어로
다시 쓸 때는 **`실행 계약 rev 1 / 기술 rev 2`**를 명기한다. metadata 없는 run은 표에 올리지 않는다.
_Avoid_: 계약, 측정 규칙 (어느 판인지 불명확)

**`PER_QUERY_CONNECTION_DIAG` (구 `G2`)**:
22쿼리를 순서대로 도는 진단 트랙이되 **쿼리마다 클라이언트 프로세스를 새로 띄우는** 트랙이다
(`g2-stream/scratch/run-g2.sh`). 따라서 그 결과를 **단일 connection 결과로 표현하지 않는다.**
ADR 0016의 버퍼 레짐 `stream`은 그대로 유효하다 — 엔진 버퍼는 연결과 무관하게 공유되기 때문이다.
한 연결에서 여러 쿼리를 도는 성질은 **미측정**이며 `TRUE_SINGLE_CONNECTION` 트랙(백로그 B1)의 대상이다.
_Avoid_: G2, 단일 세션 스트림, single-session 22쿼리

**`connection_mode`**:
run 하나가 연결을 어떻게 쓰는지다. **`per-query-connection`** = 측정 런마다 클라이언트를 새로
띄운다(wall 세트, `PER_QUERY_CONNECTION_DIAG`). **`single-connection-n-statements`** = 한 연결에서
같은 문장을 N회 실행한다(CPU 브래킷·프로파일 세트). 두 모드의 값을 같은 표에 합치지 않는다 —
연결 수립 비용과 연결 내 상태(플랜 캐시 등)가 다르다.
_Avoid_: 세션, 접속 (어느 모드인지 불명확)

**`configured-cap parity` (구 "실행 단위 파리티")**:
**노드 단위 상한**만 맞추는 파리티다 — CUBRID `parallelism=6` ↔ PG
`max_parallel_workers_per_gather=5`(+leader). **전역 예산은 맞추지 않는다**:
CUBRID `max_parallel_workers`(현 구성 **100**, `px_worker_manager_global.cpp:57-78`) ↔
PG `max_parallel_workers`(**8**)로 **12.5배 비대칭**이며 이는 제거 대상이 아니라 기록 대상이다.
`parallelism`은 `px_parallel.cpp:172 MIN (auto_degree, parallelism)`에서 **노드마다 독립 적용**되므로
중첩 병렬 스코프가 있는 플랜은 cap을 넘어설 수 있다(Q3 `USE_HASH`: 동시 활성 **12단위**,
`time-weighted active units` **7.05 = cap 6의 117.5 %**).
모든 표에 다음 5개를 **따로** 적는다: `planned` / `launched` / `동시 활성(표본 최대)` /
`time-weighted active units` / `serial tail`. 라벨은 노드 cap 대비 `time-weighted active units`로
판정한다 — **80 % 미만 `단위 붕괴` / 100 % 초과 `configured-cap 초과` / 그 사이 `병렬 유지`**,
**PG에도 같이 적용**한다. (ADR 0014+0018+0019+0020 §4)
_Avoid_: 실행 단위 파리티, 양쪽 6단위, DOP 6 (노드 cap과 전역 예산을 혼동)

**complete-case Pareto / censored lower-bound**:
`PER_QUERY_CONNECTION_DIAG`에서 **q17·q20은 양쪽 timeout, q22는 CUBRID만 timeout**이다.
따라서 순위를 낼 때 세 집합을 섞지 않는다.
**complete-case(완주 19개 = q1~q16, q18, q19, q21)**에서만 Pareto 순위를 내고, `positive_pareto`
누적 비율도 이 분모임을 명기한다. **q22는 censored lower-bound**(CUBRID ≥300.008 s / PG 0.971 s →
격차 **≥299.03 s**, 배수 **≥309x**)로 **전체 22개 우선순위 최상위**이며 lower-bound로만 인용한다.
**q17·q20은 양쪽 censored**여서 격차 판정이 불가하고 순위에 넣지 않는다.
**Q21은 "완주 19개 complete-case Pareto 1위"로만 표기**하며 "절대격차 1위"라는 무조건 서술을
쓰지 않는다(격차 59.05 s < q22의 lower-bound 299.03 s). 대치·보간은 하지 않는다. (ADR 0005+0020 §3)
_Avoid_: Pareto 1위, 최대 격차 (complete-case인지 censored 포함인지 불명확)

**상태 라벨 `measured, correctness-unverified`**:
SF1 reference answer 대조를 수행하지 않았으므로 **어떤 쿼리도 "완료"·"규명 완료"로 표시하지 않는다.**
대체 채증(엔진 간 결과값 동일성, 노드별 실제 행수 일치, 결과 행 수)은 **correctness의 하위 증거**일
