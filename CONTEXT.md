...
**공유 prepared statement (공유 준비 객체)**:
같은 DB 사용자의 여러 세션이 의미가 같은 문장의 불변 준비 결과를 공동으로 보유하는 객체다. 개별 실행의 입력·진행 상태는 포함하지 않는다.
_Avoid_: 공유 커서(개별 실행 진행 상태와 혼동), XASL 캐시(이미 존재하는 실행계획 캐시만을 가리킴)

**내부 XASL stream 경계**:
같은 서버 프로세스 안에서 컴파일 결과·계획 캐시·실행 사이를 넘기기 위해 XASL을 바이트열로 바꾸고 다시 객체로 복원하는 경계다.
_Avoid_: 계획 캐시 자체, 드라이버 프로토콜(외부 통신 경계와 혼동)


**세션 문장 핸들**:
한 세션이 준비된 문장을 식별하고 사용할 수 있게 하는 세션 소유 참조다. 공유 준비 객체와 개별 실행의 수명은 핸들의 수명과 구분한다.
_Avoid_: 공유 커서, 실행계획 자체

**준비된 문장의 실행 상태**:
준비된 문장의 특정 실행이 보유하는 입력과 진행 상태다. 같은 세션 안에서도 중첩 실행이나 여러 커서가 살아 있으면 실행 상태는 각각 구분된다.
_Avoid_: 세션 전역 scratch 하나, 공유 준비 객체
