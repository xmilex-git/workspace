...
**CAS 통합 JIRA**:
CAS 통합 작업의 변경 범위와 발견 결함, 수정 및 검증 근거를 한데 기록하고 기존 엔진·TC PR을 연결하는 단일 이슈다.
_Avoid_: 결함별 제출 묶음, 별도 결함 PR 시리즈
...
**Herdr 작업 세션**:
하나의 위임 작업을 위해 새로 만들고, 그 작업이 끝나면 닫는 전용 Herdr 세션이다. 사용자가 대화 중인 조정 대화와 수명을 분리한다.
_Avoid_: 조정 대화, Claude 대화 자체


### CAS AUTO 수용성

**AUTO 논리 세션**:
접속을 양보했다가 다시 연결해도 이어지는 사용자의 DB 세션이다. 설정·변수·준비문 등 보존할 사용자 상태의 귀속 단위이며 물리 연결과 수명이 다르다.
_Avoid_: 연결 하나, 실행 스레드, 공유 사용자 상태

**AUTO 접속 귀속 (attachment)**:
물리 연결 하나가 논리 세션과 실행자를 독점적으로 사용하는 관계다. 같은 논리 세션의 재접속도 새로운 귀속이다.
_Avoid_: 논리 세션 자체, transaction index

**AUTO 실행 슬롯**:
브로커가 동시에 수용할 수 있는 연결 실행 자원의 단위다. 예약·사용·반환 중인 자리는 점유에 포함되며 보관 중인 논리 세션의 수와 구별한다.
_Avoid_: 전체 논리 연결 수, 동시에 CPU를 사용하는 질의 수

**브로커별 세션 그룹**:
서버 안에서 한 브로커 소속 세션의 접속·양보·수용량을 관리하는 범위다. 사용자별 설정과 인증 상태를 공유하는 단위가 아니다.
_Avoid_: 브로커 공용 SQL 세션, 브로커 프로세스에 보관한 workspace
