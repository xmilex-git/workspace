...
형제가 부류를 정하지 못해 컴파일이 "게이트 확정" 으로만 표시하는 슬롯 — 형제가 전부 슬롯인 자리, 집계·분석 인자, ENUM 산술, 값 요구 함수 인자, 형제 없는 세션변수 읽기. 게이트가 실행당 1회 값 타입으로 확정한다(D-317-02·12·13). 문자 슬롯이면 collation 도 같은 자리에서 값의 collation 으로 기록된다 — 도메인과 collation 은 게이트 표의 한 항목이다. 컴파일이 타입을 정했지만 collation 이 LEAVE·ENFORCE 인 문자 슬롯(auto-param 포함)도 값 도메인을 기록하는 칸이다(`COLLATION_GATE`, #336·#338).
...
문자 형제(컬럼·리터럴·CAST·ENUM 컬럼)가 있는 연산자·함수의 직접 인자 슬롯에 컴파일이 형제 collation 을 고정하는 현행(develop) 규칙. 중첩 식 안의 슬롯까지 내려보내지 않고(D-322-02), 안쪽 식은 컴파일이 넣는 `CAST(… AS VARCHAR COLLATE c)` 래퍼(ENFORCE)로 바깥 collation 에 맞춘다. 이 캠페인은 이 규칙을 바꾸지 않는다 — ENFORCE 슬롯은 값 도메인을 기록하는 게이트 칸이고, ENFORCE 래퍼는 collation 게이트 노드다(#338).
...
_Avoid_: 해석기에 값을 넘기는 서술, 행마다 다시 분류하는 것(세션변수 읽기도 실행 시작 때 값으로 한 번 — #366 D-366-06)
...
**파생 소비자 (derived consumer)**:
다른 노드(생산자)가 쓴 값을 읽기만 하는 자리 — 값 포인터(val_list·산술 결과·누산기·분석 결과·단일 행 부질의 결과를 가리키는 regu), 리스트 위치와 정렬 키, 집합 연산·CTE 리스트 컬럼, 집계·분석의 누산기와 distinct/정렬 리스트. 도메인은 생산자에서 온다: 생산자가 게이트 확정이면 그 칸을 공유하고(ALIAS), 컴파일 확정이면 그 도메인을 로드 도출이 싣는다. 값 포인터의 컴파일 도메인은 읽는 쪽의 것이라(INSERT…SELECT 는 대상 컬럼) 값의 타입을 설명하지 못할 수 있다(F-335-07, #337).
_Avoid_: 값 포인터의 컴파일 도메인을 값의 타입으로 믿는 것, 파생 소비자마다 게이트 칸을 따로 두는 것, 첫 값을 기다려 정하는 것(늦은 바인딩)
...
**노드 칸 (node cell)** — 코드 이름 `RESOLVED_DOMAIN_TABLE.taken` · `taken_list` · `taken_type`, 항목의 `cell`:
develop 이 실행 중 노드 필드에 쓰고 XASL clear 에서 되돌리던 답(문자 LEAVE 노드의 첫 계산, 셋업이 주는 집계·분석·위치·출력 열 도메인과 연산자 타입 — 행이 정하던 D-336-E 바인딩은 #366 에서 없어짐)을 두는 XASL_STATE 의 실행별 자리. 적재가 열린 컴파일 도메인의 노드와 모든 집계·분석에 번호를 주고, 노드마다 따로다(생산자 항목을 공유하는 값 포인터도 자기 칸). 실행의 주인 스레드만 쓰고, 읽는 곳은 칸 ?: 컴파일 도메인을 읽으며, 실행 동안 남아 develop 의 쓰기·원복 이력과 같다(#355 D-355-01·09).
_Avoid_: 게이트 표(봉인된 결정)와 혼용, 플랜 노드 필드에 쓰는 것, 새 결정 지점으로 서술(develop 이 쓰던 답의 자리일 뿐이다)
...
**세션변수 문장 타입 (session variable statement type)** — 코드 이름 `DOMAIN_SESSION_VARIABLE`, `qexec_resolve_session_variables`:
문장이 읽는 세션변수가 그 문장 동안 갖는 한 타입. 실행 시작 때 값의 타입과 문장 안 대입식의 계획 도메인을 합친다 — 열이 어떤 행에서 NULL 이어도 열의 타입이고, 명시적 NULL·미정의 시작만 타입이 없다. codeset·collation 이 같은 문자열은 길이·CHAR/VARCHAR 와 무관하게 한 타입(대입하는 변수의 결정은 가변 길이 VARCHAR)이고, 그 밖의 다른 타입은 실행 전 `ER_QPROC_SESSION_VARIABLE_TYPE`(-1384)이다. G1 의 7b 단계가 정하고 행의 읽기는 그 타입만 받는다 — 게이트가 보지 못한 쓰기(저장 프로시저)의 다른 타입 값도 같은 오류다(#366, 사용자 결정 U1~U4, D-366-01~06).
_Avoid_: 행마다 타입이 바뀌는 변수(D-336-E, #366 에서 없어짐), 대입만 하고 읽지 않는 문장에 적용하는 것, 숫자끼리는 같은 타입이라는 서술(INT 에 DOUBLE 을 넣으며 읽으면 오류다)
...
**비교 기록 (comparison record)** — 코드 이름 `DOMAIN_COMPARE_PLAN`:
두 값을 비교하는 자리마다 로드가 하나씩 붙이는 기록이다. 자리는 술어 항, FIELD·NULLIF·LEAST·GREATEST 노드, LIMIT 의 row count, 머지 조인의 열 쌍이다. 두 쪽 키를 플랜이 알면 로드가 정하고, 한쪽이라도 게이트가 정하면 G1 이 실행당 1회 정한다. 정하는 것은 develop 비교의 내용이다: 어느 쪽을 무엇으로 변환하는지, 어떤 cmpval 을 쓰는지, collation, 변환이 실패할 때의 답. 행은 kernel 만 돈다(D-352-01·02, D-354-03~05).
_Avoid_: 비교 도메인을 피연산자 regu 항목이나 `SCAN_ID` 에 두는 서술

**키 쌍 표 (key pair table)**:
값이 가질 수 있는 모든 키 쌍의 비교를 행 전에 정해 둔 한 벌의 표다. 키는 원소 타입이고, 문자열·ENUM 이면 등록된 collation 까지 포함한다. 강제 변환 있음·없음 두 모드를 담는다. 키를 데이터만 아는 비교가 이 표를 읽는다: 컬렉션 원소, JSON 스칼라, 파티션 경계, 해시 그룹 키, 두 ORDERBY_NUM 항의 상한, 스트림의 리터럴 아닌 쪽. 이들은 두 값의 키로 칸을 찾아 읽을 뿐, 결정하지 않는다(D-354-01, D-352-07 의 모든 키 규칙).
_Avoid_: 키를 플랜이 아는 비교(술어 항·산술 노드)에서 이 표로 값의 타입을 찾는 것, 행에서 처음 만난 쌍을 그때 정해 담는 캐시
