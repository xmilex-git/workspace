...
_Avoid_: execute 직전 잔여 확정(이전 캠페인 용어), 세 번째 결정 지점(PX 워커), 플랜 안의 DB_VALUE 를 바꾸는 제자리 coerce
...
변환 시점을 정하는 세 부류. 상수 부류(리터럴 = 컴파일 폴딩, 슬롯 = 게이트 1회) · 행 의존 부류(컬럼·컬럼 식 = 계획된 변환기로 행마다) · 상관 부류(외부 행 값·비상관 서브쿼리 결과 = 소비 블록 시작/range open 시 스코프당 1회)(D-317-17).
_Avoid_: "행당 변환 0회" (목표는 행당 결정 0회)

**계획된 변환기 (planned converter)**:
컴파일(또는 게이트)이 술어·산술·키 원소마다 계획에 고정한 (도메인, 피연산자별 원 타입→목표 타입 함수 ID). 실행은 타입 판정 분기 없이 이 함수만 부른다. 변환 방향과 도메인은 현행 서버 표를 그대로 옮긴 것이다(D-317-16·19).
_Avoid_: 실행 시 rank 판정·타입 switch 를 거치는 변환
...
슬롯(호스트 변수·auto-param·세션변수 읽기)의 타입을 같은 식에서 타입이 알려진 형제(컬럼·리터럴·CAST 결과·PL 선언 타입)로부터 확정하는 규칙이다. 비교·대입 자리에서는 이미 현행 규칙이며, 이 캠페인은 산술·함수 인자·공통값·집계 자리까지 확장한다(D-317-01).
...
형제 미러의 우선순위. 식 결과의 소비자(대입 대상 컬럼·비교 상대)가 정한 기대 도메인이 중첩 식 안의 슬롯까지 먼저 닿고, 리터럴 형제 미러는 소비자 도메인이 없을 때만 적용된다 — `INSERT decimal(10,5) VALUES (IFNULL(?, 0))` 의 `?` 는 리터럴 `0` 이 아니라 decimal 을 따른다(D-327-11). 타입 축에만 있는 규칙이며 현행 파서가 이미 이렇게 동작한다.
...
**게이트 표 (gate table)**:
XASL_STATE 에 사는 실행별 항목 — 슬롯 ID → (도메인, collation, 변환기). 게이트 확정 슬롯과 그 파생 소비자(산술 결과·리스트 컬럼·누산기·정렬 키·비교 도메인)의 실행당 답이 여기에만 있고 플랜에는 없다. PX 워커는 이 표를 값 배열과 함께 깊은 복사로 상속한다(D-318-03·06).
_Avoid_: 플랜 노드의 `domain` 필드(불변), 바인드 값 배열과 혼용

**실행 게이트 / 블록 게이트 (G1 / G2)**:
서버 게이트의 두 단계. 실행 게이트(G1)는 실행당 1회, 값 배열 형성 직후에 슬롯 값을 바인드 도메인으로 변환하고 게이트 확정 슬롯과 파생 소비자를 게이트 표에 채우는 유일한 결정 지점이다. 블록 게이트(G2)는 XASL 블록마다 aptr 실행 뒤·스캔 open 전에 상관 부류(aptr 결과·비상관 서브쿼리 결과)에 계획된 변환기를 적용할 뿐 결정하지 않는다(D-318-08).
_Avoid_: G2 를 두 번째 결정 지점으로 서술, 스캔 open 마다 도메인을 정하는 것
