...
#
# ─────────────────────────────────────────────────────────────────────────────
# ctp-run FORK of cubridci docker-entrypoint.sh
#   upstream: CUBRID/cubridci @ test_rl8.10, commit 89d1dff
#   image:    cubridci/cubridci:test_rl8.10 (bind-mounted over /entrypoint.sh,
#             so the image itself is never rebuilt — see the ctp-run skill)
#
# Local additions (everything else is upstream, verbatim):
#   1. apply_ctprun_overrides() — CTP_SCENARIO / CTP_EXCLUDE work for EVERY
#      category, not just the shell variants (upstream has SHELL_SCENARIO /
#      HA_SCENARIO only, and no knob at all for sql/medium/isolation/jdbc).
#   2. testcase_update_yn=false is FORCED for every category. Upstream derives
#      the shell confs from shell_ci.conf, which ships testcase_update_yn=true:
#      CTP then git-pulls the testcase repo INSIDE the container, which both
#      destroys the materialized per-shard subset and silently switches the run
#      to develop — the exact failure this runner exists to prevent.
#   3. provenance is echoed at the head of the run (CTPRUN_PROVENANCE).
#   4. `test` no longer requires `checkout`: CTP and the testcases are always
#      bind-mounted from the host (worktree copies), so nothing is cloned here
#      and no GitHub token ever enters the container.
# ─────────────────────────────────────────────────────────────────────────────
set -o pipefail
...
# ctp-run addition. Upstream lets only the shell variants move their scenario
# (SHELL_SCENARIO) and ha_repl its own (HA_SCENARIO); sql, medium, isolation,
# sql_by_cci and jdbc always run whatever the stock conf points at. This applies
# both knobs uniformly to the conf the category resolved to, AFTER any
# CONF_WRITER has run, and verifies every key it claims to have set — an
# upstream key rename must fail loudly, never silently run the whole suite.
#
# CTP_SCENARIO : absolute scenario root for this run (unset = leave the conf's).
# CTP_EXCLUDE  : exclusion-list path. Set-but-empty means "no exclusions" (CTP
#                skips an empty key and dies on a missing file), so the test is
#                on being set at all, not on being non-empty.
function apply_ctprun_overrides ()
{
  local conf="$CTP_HOME/$CTP_CONF"
  [ -f "$conf" ] || { echo "** ERROR: $conf not found; cannot apply ctp-run overrides" >&2; exit 1; }
  [ -w "$conf" ] || { echo "** ERROR: $conf not writable; the CTP mount must be rw" >&2; exit 1; }

  local -a want=()
  local -a seds=()

  if [ -n "${CTP_SCENARIO:-}" ]; then
    seds+=( -e "s|^scenario=.*|scenario=$CTP_SCENARIO|" )
    want+=( "scenario=$CTP_SCENARIO" )
  fi
  if [ -n "${CTP_EXCLUDE+set}" ]; then
    seds+=( -e "s|^testcase_exclude_from_file=.*|testcase_exclude_from_file=$CTP_EXCLUDE|" )
    want+=( "testcase_exclude_from_file=$CTP_EXCLUDE" )
  fi
  # Always: never let CTP update the testcases under us (see fork header, note 2).
  seds+=( -e "s|^testcase_update_yn=.*|testcase_update_yn=false|" )

  if [ "${#seds[@]}" -gt 0 ]; then
    sed "${seds[@]}" "$conf" > "$conf.ctprun" && mv -f "$conf.ctprun" "$conf"
  fi

  # testcase_update_yn is absent from some confs (sql.conf); false is the CTP
  # default there, so only enforce the key when the conf actually carries it.
  if grep -q '^testcase_update_yn=' "$conf"; then
    want+=( "testcase_update_yn=false" )
  fi

  local key
  for key in "${want[@]}"; do
    grep -qxF "$key" "$conf" \
      || { echo "** ERROR: $CTP_CONF lacks '$key' after the ctp-run override; check the upstream conf keys" >&2; exit 1; }
  done
  echo "[ctp-run] conf $conf: ${want[*]:-no override}"
}

# ctp-run addition: one line saying exactly what this container is verifying, so
# a result can always be traced back to install / image / CTP / testcase ref.
function print_ctprun_provenance ()
{
  [ -n "${CTPRUN_PROVENANCE:-}" ] || return 0
  echo "[ctp-run] provenance: $CTPRUN_PROVENANCE"
}
...
function run_test ()
{
  ...
  # ctp-run: bind-mounted by the orchestrator, never cloned in here.
  [ -d "$CTP_HOME" ] \
    || { echo "** ERROR: no CTP at $CTP_HOME; the orchestrator must mount a CTP copy" >&2; exit 1; }
  [ -d "$WORKDIR/$TC_REPO" ] \
    || { echo "** ERROR: no testcases at $WORKDIR/$TC_REPO; the orchestrator must mount a testcases worktree copy" >&2; exit 1; }
