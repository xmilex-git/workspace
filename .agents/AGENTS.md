...
- **GPT/Codex → Claude Sonnet worker through the `herdr` skill.** Read that skill before delegation and follow its pane creation, agent startup, prompt, wait, and result-reading workflow. Start the worker with `herdr agent start <unique-name> --kind claude --pane <returned-pane-id> -- --model sonnet`. Pass the target checkout and task scope explicitly in the prompt. The lead must be running inside Herdr (`HERDR_ENV=1`); if the skill or Herdr session is unavailable, report the delegation blocker rather than silently falling back to a Codex subagent or direct lead execution.
