...
- **GPT/Codex → Claude Sonnet worker through the `herdr` skill.** Read that skill before delegation and follow its pane creation, agent startup, prompt, wait, and result-reading workflow. Start the worker with `herdr --session <selected-name> agent start <unique-name> --kind claude --pane <returned-pane-id> -- --model sonnet`. Pass the target checkout and task scope explicitly in the prompt. Codex may run outside Herdr: follow the skill’s external-coordinator workflow, prefix every control command with `herdr --session <selected-name>`, and use explicit pane IDs. Do not synthesize `HERDR_ENV`. If the skill or target session is unavailable or ambiguous, resolve that blocker before delegation. After reading and preserving the final report, close worker panes created for the completed task as described in the skill.
