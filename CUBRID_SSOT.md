...
14. **(2026-08-14) `CUBRID_TMP` 불일치는 "Could not connect to master server"로 나타난다** — cubrid 유틸리티는 유닉스 소켓을 `$CUBRID_TMP`에서 찾으므로, master/서버를 띄운 프로세스와 다른 `CUBRID_TMP`를 가진 셸에서는 server stop 등 제어 명령이 접속 실패한다. 제어 명령 전에 대상 프로세스의 기동 당시 값과 일치시킨다(`tr '\0' '\n' < /proc/<pid>/environ | grep CUBRID_TMP`). 또한 `cubrid service stop`으로 master를 먼저 내리면 진행 중이던 server stop 요청이 유실돼 cub_server가 고아로 남을 수 있다 — **server stop은 프로세스 부재 확인 후에 service stop** 순서로. 고아 cub_server 정리는 kill -9가 아니라 SIGTERM(graceful shutdown 경로).
15. **(2026-08-07) 대용량 DB를 새로 적재할 땐 인덱스 단계를 분리해 loaddb `--no-logging-index`로 태운다** (PR #7504 / CBRD-27071, develop `1aa79a03b`). 스키마(인덱스 제외) → 데이터 → **인덱스 파일** 3단계로 나누고 마지막 단계만 `cubrid loaddb -i <index.sql> --no-logging-index <db>`로 적재하면 no-redo 병렬 벌크 빌드가 발화한다(341M행 기준 609.2s → 245.5s, 2.48×). 발화 조건이 좁다: **서버가 떠 있는 CS 모드**여야 하고(SA 모드는 플래그를 무시한다 — `load_db.c:842`), 서버가 loaddb 계열 client type의 요청만 허용하며(`network_interface_sr.cpp:4729`), `-i` 인덱스 로드 구간에만 적용된다. PK/FK를 `-s` 스키마 파일에 남겨두면 이 경로를 타지 못하고 데이터 적재 중 인덱스 유지 비용까지 그대로 낸다. WITH ONLINE 인덱스와 정렬 런이 부족한 소형 테이블은 옵션과 무관하게 기존 로깅 빌드로 폴백한다. **적재 후에는 전체 백업을 새로 받는다** — 로그 체인에 barrier가 남아 이전 백업 체인으로는 그 지점을 넘는 replay가 거부된다(`log_recovery.c:3272`).
16. **(2026-08-26) 위임 에이전트의 백그라운드 빌드는 "알림 대기"로 턴을 끝내면 안 된다 — 완료 감지는 push(알림)가 아니라 pull(폴링)이 계약이다.** 한 세션에서 sonnet 서브에이전트가 `run_in_background` 빌드를 걸고 "완료 알림을 기다리며" 턴을 끝냈다가, 알림이 유실되거나 미처리되어 **4회 연속 10–60분씩 idle 스톨**한 인시던트(리드가 수동 메시지로 깨울 때까지 무진행). 메커니즘: 백그라운드 작업 완료 재호출은 push 채널 단일 의존이고, 이 채널은 세션 간에서 손실될 수 있다 — pull 폴백이 없으면 스톨이 구조적으로 재발한다(13번 nohup 침묵사와 같은 부류: 완료 판정을 프로세스/알림에 위임하지 말 것). 정석: 빌드를 백그라운드로 건 **같은 턴 안에서** 종료 마커를 유계 폴링한다 — `timeout 540 bash -c 'until grep -q "^installed " <build.log>; do sleep 15; done'` (필요 시 반복 호출; 마커는 justfile 빌드의 마지막 줄 `installed <mode> (...) -> <prefix>`, 또는 설치된 `cub_server` mtime > HEAD 커밋 시각). 리드 측 의무 두 가지: (a) 빌드→테스트 파이프라인을 위임하는 **프롬프트에 이 폴링 계약을 명시**한다, (b) 서브에이전트의 idle 신호를 진행 증거로 믿지 말고 산출물 타임스탬프(`stat` 설치 바이너리 vs 커밋 시각)로 직접 검증한다.
17. **(2026-08-27) 10분 내에 끝나는 유한 작업(증분 빌드·unit·smoke)은 백그라운드+폴링이 아니라 포그라운드(블로킹)로 실행하는 것이 기본이다 — 16번 폴링 계약을 지켰다고 믿은 워커의 자체 모니터가 불발해 빌드 완료 후 2시간 40분 idle 스톨한 인시던트(실작업은 라운드당 ~10분): 위임 프롬프트에 "전 단계 포그라운드 체이닝 + idle 전 同턴 보고"를 명시하고, 리드는 워커 idle 알림을 무시하고 산출물 mtime이 ~10분 정지하면 즉시 개입한다.** 2차 재발(2026-08-27, wf122/A6 게이트: install 완료 후 재스톨) 원인 분석: 이 규칙이 SSOT에만 있어 **위임 프롬프트를 쓰는 시점의 컨텍스트에 강제되지 않았다** — 리드가 SSOT 미로드 상태로 위임하면서 오히려 nohup+백그라운드 패턴을 프롬프트에 직접 써줬다. 구조 조치: 이 계약을 항상 자동 로드되는 `.claude/CLAUDE.md`의 분업(Division of labor) 조항 바로 아래에 미러링("Delegation execution contract") — 위임 프롬프트에 문구 포함이 의무이며, 누락된 위임은 이 인시던트의 재발 경로다.
17. **(2026-08-26) 서버를 띄우기 전 포트를 레지스트리에서 클레임한다** — 동시 Claude 세션 간 `cub_master`/broker 포트 충돌 방지. `just port-claim <db> <effort>`가 1700–1799에서 빈 `cubrid_port_id`와 broker 100-블록을 자동 배정해 `.git_ignored_dir/port-registry/claims.md`(기계-로컬, git-ignored)에 기록한다. 현황은 `just ports`(클레임 + 라이브 리스너), 작업 종료 시 `just port-release <db|port>`. 고정 점유 포트(1523 기본, 1568 `~/CUBRID` 설치본, broker 30000/33000대)는 클레임 금지. 프로토콜 상세는 `docs/agents/port-registry.md`.

18. **(2026-09-04, 2026-09-19 갱신) TPC-H(SF10) 자산은 매번 찾아다니지 말고 여기 적힌 것만 쓴다.** **2026-09-19 확인: 기존에 이 항목이 단일 진실로 지정했던 `~/databases/tpch/` 와 DB `tpch_sf10_q1` 은 이 호스트에 더 이상 존재하지 않는다** — `~/databases/tpch/`, `tpch_sf10_q1` 볼륨, `queries/`, `schema/`, `conf/cubrid.conf.tpch-b8192`, `load_tpch.sh`, `~/databases/tpch/README.md`, 보존본 `.git_ignored_dir/tpch-sspq/cubrid-databases/` 가 모두 없다(전수 확인). 현재 이 호스트에 남아 있는 SF10 자산은 다음이 전부다 — DB `tpch_sf10`(`CUBRID_DATABASES=~/databases/pr7866`, 그 디렉터리의 자체 `databases.txt` 로만 등록, 22G), 질의 `~/dev/workspace/tpch-sspq/queries/qN-cubrid.sql`(Q15 는 create_view/select/drop_view 3파일), 측정 conf `.git_ignored_dir/conf/pr7866.conf`(data_buffer 16G·`double_write_buffer_size=0`, 포트는 `just port-claim <db> <effort>` 값으로 교체; 머신 로컬이라 git 추적 대상이 아님), 하네스 `tpch-sspq/harness/`. **`tpch_sf10` 의 적재 시점과 적재에 쓴 빌드는 어디에도 기록돼 있지 않다. 그리고 CREATE TABLE 스키마(`create_tpch_table.sql`)와 `load_tpch.sh` 가 호스트에 없어, 지금은 무(無)에서 재적재하는 경로가 없다** — 재적재가 필요하면 절차 자체는 `docs/research/cbrd27365-perf-193.md` §2b 에 남아 있으므로 거기서 스크립트를 먼저 복원해야 하고, 그 복원 비용을 측정 일정에 포함시켜야 한다. **온디스크 포맷을 바꾸는 develop 커밋(예: CBRD-24094 비유니크 오버플로 헤더 8B→16B, 2026-08-27) 뒤에는 측정 DB 를 측정 대상 빌드로 재적재해야 한다** — 새 코드는 레거시 체인을 손상으로 취급하고 release 빌드는 assert 없이 오독한다(#193 인시던트: 옛 DB 로 A 패스 6개 질의를 잰 뒤 폐기). 적재 이력이 남아 있지 않은 `tpch_sf10` 에는 이 규칙이 더 강하게 적용된다 — 포맷 무관한 측정(바이너리만 교체하고 재적재가 불필요한 A/B)에만 그대로 쓰고, 그 밖에는 먼저 재적재하라. Notion 기준선 절대값과의 비교는 참고용, 판정은 같은 호스트 A/B.
...
사용자 방침(2026-09-22): CUBRID 단위 테스트는 앞으로도 실행·활성화하지 않는다. 기존 지도·티켓·스킬의 unit 게이트를 대체하며, 빌드·컨테이너 CTP·해당 작업의 런타임 검증으로 확인한다. 이후 사용자가 단위 테스트를 명시적으로 요청하면 그 요청을 따른다.
