...
17. **(2026-08-27) 10분 내에 끝나는 유한 작업(증분 빌드·unit·smoke)은 백그라운드+폴링이 아니라 포그라운드(블로킹)로 실행하는 것이 기본이다 — 16번 폴링 계약을 지켰다고 믿은 워커의 자체 모니터가 불발해 빌드 완료 후 2시간 40분 idle 스톨한 인시던트(실작업은 라운드당 ~10분): 위임 프롬프트에 "전 단계 포그라운드 체이닝 + idle 전 同턴 보고"를 명시하고, 리드는 워커 idle 알림을 무시하고 산출물 mtime이 ~10분 정지하면 즉시 개입한다.**
