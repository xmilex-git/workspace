...
16. **(2026-08-26) 서버를 띄우기 전 포트를 레지스트리에서 클레임한다** — 동시 Claude 세션 간 `cub_master`/broker 포트 충돌 방지. `just port-claim <db> <effort>`가 1700–1799에서 빈 `cubrid_port_id`와 broker 100-블록을 자동 배정해 `.git_ignored_dir/port-registry/claims.md`(기계-로컬, git-ignored)에 기록한다. 현황은 `just ports`(클레임 + 라이브 리스너), 작업 종료 시 `just port-release <db|port>`. 고정 점유 포트(1523 기본, 1568 `~/CUBRID` 설치본, broker 30000/33000대)는 클레임 금지. 프로토콜 상세는 `docs/agents/port-registry.md`.
