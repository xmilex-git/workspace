...
~~#144 성능 회수 로드맵~~ ✅ **완주·close(2026-07-06)** — 착지: P1-2 리더 LRU(6af3965ae)·P3 D1+D2(936850f83+855771c8b, 중형 조인 −40%). outer_join 2.42×(잔여=w5 #141 귀속). 반증 종결: P2·P3단독. 분리: #145(파이프라인 융합 설계, 착수 승인 게이트). 4슬롯 유휴 | — | 게이트 6종/슬라이스, #142가 성능 기준선 |
| `.32` | **유휴** (#144 완주·close) | Opus | 다음 dispatch 대기 |
...
## 9. Supervisor 승계 노트 (2026-07-06 — 본 세션(Fable) 한도 99% 소진으로 작성)

**인수 방법**: 이 문서 + SSOT(#75 본문)를 읽고 "supervisor mode로 동작하라". 모델은 opus여도 무방(dispatch·리뷰·판정 업무).

**진행 중인 것 (인수 시 최우선 확인)**:
1. **#144 성능 회수** — .32(opus)가 P1-1(프로파일)부터 실행 중. 항목당 #144에 보고 → 리뷰 → 체크박스 진행. **P3은 워커의 설계 코멘트에 supervisor 승인 필요**(#91 재론). P1-4는 조사 보고만 받고 구현은 별도 상신.
2. **#143 마감** — 문서 동기화(.30, task_143_docs.md, docs-only)가 진행 중이었음. 착지 보고 오면 diff가 문서만인지 확인 후, **사용자 최종 확인을 받고 #143 close**.
3. 워처: `.not_git_tracking/scratch/sup_watch.sh` — 대상 이슈 번호를 파일 상단 `for n in ...` 두 곳에서 갱신 후 `run_in_background`로 실행(현재 #143 — #144 추가 필요). pkill과 기동을 같은 명령에 섞지 말 것(자충수 전례).

**열린 판단 대기**: #113 upstream PR(ready-for-human), #52~56/59/61, perf 보류 #33~35/58(→#144 P4와 교차), #141(#144에 편입 — 추적용 유지).

**이 세션에서 확립된 관행(§0 외 추가분)**: 게이트5 성능 판정 = 동일호스트 A/B 비율(절대치 금지) / 대블록 rename·이동·주석은 기계 검증기(comment_only_diff.py 등 — 엔진 contrib·scratch에 있음) / 워커 dispatch 후 "esc to interrupt" 확인까지가 dispatch / 한글 IME 미전송 텍스트는 C-u 후 -l 재주입.
