...
SELECT id, grp, n, SUM(n) OVER (PARTITION BY grp ORDER BY n) s, AVG(d) OVER (PARTITION BY grp ORDER BY n ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) a FROM t06 ORDER BY grp, n, id;
...
SELECT grp, MEDIAN(n), PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY d), PERCENTILE_DISC(0.25) WITHIN GROUP (ORDER BY k) FROM t06 GROUP BY grp ORDER BY grp;
SELECT id, grp, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY n) OVER (PARTITION BY grp) pc, PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY k) OVER (PARTITION BY grp) pd FROM t06 ORDER BY grp, id;
...
SELECT id, i, GROUP_CONCAT(k ORDER BY k SEPARATOR '|') OVER (PARTITION BY grp) gc FROM t06 ORDER BY id;
DROP TABLE t06;
