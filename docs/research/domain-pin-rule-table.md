...
| B14 | `enum_col < ?` | ENUM 도메인 | 위와 같음(서수 비교) | C | 현행; TIME 바인드 2행은 결함 → 값 A/B 로 확정 후 -494 | 결함 1건 (D-335-06·08: 바인드 캐스트는 클라이언트가 develop 처럼 하고 ENUM 은 캐스트하지 않는다(CUBRIDSUS-9007) — #336 은 ENUM 도메인 슬롯을 게이트 슬롯으로 두어 불변식만 지키고, 이 결함은 게이트 ENUM 변환기(dpin-14, 실행 측 비교 변환)에서 고친다) — **#358: 결함 아님** — 리터럴 `enum_col < time'…'` 과 같은 답(ENUM 라벨을 TIME 으로 바꿔 견준다, 시간으로 안 읽히는 라벨은 -181), 후속 [#360](https://github.com/xmilex-git/workspace/issues/360) |
...
|---|---|---|---|---|---|---|
| U0 | `INSERT col ← ?`, `UPDATE SET col = ?`, MERGE INSERT/UPDATE — 대입 대상 도메인은 VALUES/SET 식 안의 슬롯까지 **소비자 도메인 우선**으로 닿는다(`VALUES (IFNULL(?, 0))` decimal(10,5) → decimal, D-327-11) | 컬럼의 완전한 도메인(현행) | C | K 게이트: 현행 대입 캐스트(정수 ← 1.5 **반올림 2**, 문자 파싱, 날짜 ← 숫자 -494) | 현행 | `INSERT int_col ← ?` DATE 바인드가 오류 없이 0행이던 현행은 결함 → -494 (D-317-15 정정) — **#358: 현행이 이미 -494**(csql·JDBC; 0행은 #317 탐침 파서가 EXECUTE 의 stderr 오류를 놓친 측정 오류), 변화 없음 |
...
| U13 | `COALESCE(CAST(? AS DATETIME), CAST(? AS DATETIME), ?)` | 형제 DATETIME 미러; 비문자 결과 도메인에 collation 플래그 금지 → **게이트 확정(값 도메인, develop 격자) — D-336-B(2026-09-24)** | G | K 게이트 | 현행 값; optdebug assert(L-19) 소멸 | 결함 소멸 — **철회(D-336-A)**: 답 불변(develop) — **#358**: develop 에 L-19 결함이 없다(csql·JDBC, 두 빌드). dpin 은 NULL 인 상수 피연산자를 계획 도메인으로 접어 답이 develop 과 다르다 → [dpin-17g #364](https://github.com/xmilex-git/workspace/issues/364) — **#364**: 게이트가 상수 부분트리 피연산자를 그 값의 도메인으로 접는다(G1 7단계, develop 답, TC `common_value_constants`) |
...
| F2 | `to_char(?, ?)`, `to_char(col, ?)`, `to_char(?)` | 게이트 확정(값 슬롯·포맷 슬롯) | G | 게이트 | 현행; `to_char(dtt, ?)` BIGINT assert(D5) → 오류 | 결함 소멸 — **#358: 남는다**: 포맷 미러가 D-336-B 로 철회됐고 결함 자리는 `date_to_char` 의 포맷 타입 검사다(release 는 서버 SIGSEGV, 리터럴 포맷은 -783) → develop 결함 [#363](https://github.com/xmilex-git/workspace/issues/363) |
...
| S5 | `@v` 읽기 — 형제 없음(`SELECT @v`, `sum(@v)`) | 게이트 확정(초기 저장값 타입); 값은 휘발 — 실행 중 저장 타입이 바뀌면 게이트 도메인으로 변환(실패 정책 X1; 타입이 바뀐 행에서만 표 재조회 D-325-10) — **판정 완료(D-325-12)**: P-S1·S6·S9 불변, P-S8 `sum(@v)` 은 `yes` 모드에서 오류 → 값(`domain-pin-converters.md` §5) — **#366(사용자 결정 U1~U4, D-336-E 대체)**: 문장이 읽는 변수의 타입은 문장 동안 하나다 = 시작 값 타입 + 대입식의 계획 도메인(열이 행에서 NULL 이어도 열의 타입). 다른 타입은 CAST 없이 실행 전 -1384, codeset·collation 이 같은 문자열은 한 타입(결정은 가변 길이 VARCHAR). 내용 부류는 시작 값의 부류(D-366-06) | G | 게이트 | 현행 | **있음**(`yes` 모드 P-S8) |
...
**확인(#358, 2026-09-26 — develop `c63a3b993` · dpin `f4b8b5e72`, optdebug·release, csql -S·JDBC)**: 위 "고친다" 가운데 D2·D3(#340)·D4(#337·#341)·D6(#341)·`char(n)_col = ?`(#335)는 닫혔고 D7 은 [#353](https://github.com/xmilex-git/workspace/issues/353) 으로 나갔다. 해소 기록이 없던 넷의 결과는 이렇다. **D5** `to_char(날짜·시간, ?)` 에 문자열이 아닌 포맷 바인드 — develop·dpin 이 같다(optdebug assert, release 는 csql·cub_server SIGSEGV; 리터럴 포맷은 -783). 결함 자리는 `date_to_char` 의 포맷 타입 검사로, 이 지도가 바꾼 경로가 아니고 없애려던 포맷 미러(F2)는 D-336-B 로 철회됐다 → **develop 결함 [#363](https://github.com/xmilex-git/workspace/issues/363)**(캠페인 밖, #353 선례). **L-19(C15·U13)** — develop 에 assert·CAS 사망이 없다(이전 캠페인 변경의 산물이었다) → 해당 없음. 같은 문장에서 dpin 의 게이트 결정이 develop 과 달랐다(develop 은 NULL 값을 타입 없이 접는다) → **[dpin-17g #364](https://github.com/xmilex-git/workspace/issues/364)** 가 고쳤다(`406e1ed8f`: 상수 부분트리 피연산자는 값의 도메인으로 접는다 — develop 답; 행이 주는 피연산자는 계획 도메인, D-340-01). **`INSERT int_col ← ?` DATE 0행(T28·T29)** — 결함 아님: develop 도 -494(대입)·-181(`INSERT … SELECT ?`)로 멈춘다. 0행은 #317 탐침 파서가 EXECUTE 의 stderr 오류를 라벨에 붙이지 못한 측정 오류다. **`enum_col < ?` TIME 2행(T12·B14)** — 슬롯 결함 아님: 리터럴 `enum_col < time'…'` 과 같은 답이다(ENUM 라벨을 TIME 으로 바꿔 견준다) → 후속 [#360](https://github.com/xmilex-git/workspace/issues/360) 항목. 탐침·결과: [#358 해소](https://github.com/xmilex-git/workspace/issues/358).
...
## 6. collation 축 (짝 티켓 #322, D-322-01~04; 구현 #338 D-338-01~07)

**개정(#338, 2026-09-24 — 사용자 결정 Q2 "답은 develop, collation 도 게이트가 값에서 정한다")**: 아래 원칙 문단의 "컴파일 ENFORCE·LEAVE 0" 은 쓰이지 않는다. **컴파일은 develop 의 collation 추론(형제 ENFORCE·LEAVE)을 그대로 둔다** — 스트림에 `TP_DOMAIN_COLL_LEAVE`·`ENFORCE` 가 남고, 로드 도출이 그 항목을 모두 게이트 칸으로 덮는다(D-338-01): LEAVE·ENFORCE 문자 바인드 슬롯(auto-param 포함)은 `COLLATION_GATE` 슬롯이 값 도메인을 기록하고, 컴파일이 타입을 정한 문자 식 노드(산술·함수·CAST)는 **collation 게이트 노드**로 G1 이 피연산자의 결정 도메인에 연산자의 실행 규칙(병합 `LANG_RT_COMMON_COLL` · 첫 문자 인자 · 서식 인자 · LANG_SYS · 분기, CAST LEAVE/ENFORCE·TO_CHAR 규칙 — 해석기 `domain_resolve_character`)을 실행당 1회 적용해 정하며, 파생 소비자(위치·값 포인터·집계·리스트 컬럼)는 생산자 칸을 읽는다. 경계 (a) 는 덮이지 않은 LEAVE·ENFORCE 를 거부한다(X-11 삭제). 표의 "확정" 열 C 는 develop 의 컴파일 ENFORCE 이고, 값이 collation 을 주는 자리는 전부 G 다. **-1150/-622 의 시점은 develop 그대로다** — 병합이 실패하거나(D-338-02) 행이 고르는 분기의 도메인이 다르면 게이트는 결정을 남기지 않고 행 계산이 develop 대로 오류를 내거나 값을 읽는다(C3·C9 "시점만 게이트" 는 쓰이지 않는다). **문자 결과의 precision 은 값이 정한다**(D-338-03, 사용자 승인 2026-09-24 "(A) floating"): 게이트 결정은 floating(VARCHAR 1073741823, CHAR -1)이고 피연산자 도메인으로 정해지는 것(CONCAT 합·CAST·SUBSTRING/TRIM 원천·MD5/SHA1/UUID 고정 길이)만 그 길이다 — 답 변화는 집합 연산 등 리스트 파일을 거친 컬럼의 `typeof()` precision 표시뿐(값·비교·정렬·collation·오류 불변, CTP 0건). 결과·결정 기록: [#338 해소](https://github.com/xmilex-git/workspace/issues/338).

**개정(#343, 2026-09-26 — 사용자 결정 D-343-01 "(다) MySQL 방식")**: 위 개정의 "게이트는 결정을 남기지 않고 행이 develop 대로 읽는다"(D-338-02)는 쓰이지 않는다. 게이트는 모든 문자열을 정한다. (1) 피연산자 collation 이 병합되지 않으면 **값 없음**이다 — 행 계산이 develop 시점에 -1150/-622 를 내고(오류가 값보다 먼저 난다) 소비자는 값 없는 결정의 규칙(D-341-01)을 따른다. (2) 행이 고르는 분기(ELT, IF·CASE·DECODE)의 가지 도메인이 다르면: ELT 의 인덱스가 게이트에서 값(바인드·리터럴)이면 그 인덱스가 가리키는 가지의 도메인(develop 답), 그 밖이면 가지들의 collation 을 `LANG_RT_COMMON_COLL`(값을 병합하는 develop 규칙)로 합친 한 도메인이고 행은 고른 값을 그 도메인으로 바꾼다(계획된 변환기) — 합칠 수 없으면 **실행 전 -1150**. MySQL 이 ELT 결과 collation 을 prepare 에서 인자들로 합쳐 고정하고 고른 값을 바꾸는 것과 같은 뜻이다(`Item_func_elt`). develop 은 행마다 고른 값의 collation 을 그대로 두고 그 열을 정렬·DISTINCT 하는 소비자가 첫 행이 고른 가지의 collation 을 따랐다 — 답 변화는 행 의존 인덱스 ELT 의 가지 collation 이 서로 다른 문장뿐이다 — 행이 문자열로 바꾸는 숫자 가지도 합친 도메인을 받는다(develop 은 그 값에 시스템 collation, 예 `utf8_bin`). CTP 0건이고, IF·CASE·DECODE 는 develop 컴파일이 가지를 한 collation 으로 맞춰(세션변수 가지 포함 — TC `branch_collations` G절) 해당 없음. (3) 함수의 문자 피연산자가 몇 개든 게이트 링크가 모두 든다(링크 초과 결정 없음 폐기). 결정 기록: [#343 D-343-01](https://github.com/xmilex-git/workspace/issues/343#issuecomment-5835671943).

**원칙(원문 — 위 개정이 앞선다)**: 도메인 축과 collation 축은 코드가 다르지만(L-14) **확정 지점은 같다** — 컴파일 또는 게이트, 게이트 뒤 결정 0. 문자 형제(컬럼·리터럴·CAST·ENUM 컬럼)가 있으면 컴파일이 형제 collation 으로 ENFORCE(현행 `pt_coerce_node_collation`), 형제가 전부 슬롯이면 **게이트 확정**(D-322-01): 게이트가 바인드 값들의 collation 에 현행 실행 병합 규칙 `LANG_RT_COMMON_COLL`(같으면 그것 / 한쪽 coercible 이면 다른 쪽 / 둘 다 coercible 이면 ISO 바이너리 / 둘 다 비-coercible 이면 -1150 / codeset 변환 불가 -622)을 실행당 1회 적용해 슬롯·연산자 결과·리스트 컬럼·누산기 collation 을 게이트 표에 넣는다. coercibility 8-레벨(#313 §3.1)·컴파일 병합 `pt_common_collation`·실행 병합 규칙 자체는 **변경 없음** — 실행 병합의 위치만 행 평가 → 게이트. `TP_DOMAIN_COLL_LEAVE` 는 XASL 에서 사라진다(계획 도메인의 collation_flag 는 항상 NORMAL 이거나 게이트 슬롯 참조) → #314 §4 의 쌍 조건 26곳은 타입 축과 함께 삭제(G-04 충족). 슬롯의 기본 collation 이 `LANG_SYS`(로케일 바이너리)이지 클라이언트 collation 이 아니라는 현행(#313 §3.5)은 게이트 확정 아래에서 "바인드 값이 가져오는 collation(CAS 가 세션 collation 으로 만듦)" 으로 대체되며 관찰 가능한 답은 같다(실측 A2 `COLL.hv_after_set_names`, 프로브 C1·C3·C5).
...
| C14 | 보간 정렬 키 `percentile_cont … ORDER BY varchar_col`, `median(varchar_col)` | 정렬 키 collation = 컬럼 collation(컴파일). 타입은 F10 DOUBLE(D-335-10; 분석 정렬 키 `cmp_dom` 도 컴파일 도메인). **#362**: 값 인자·게이트 의존 문자의 분석 정렬 키는 게이트 결정 부류를 정렬 전에 받고, 세션변수 읽기는 첫 비교 값이 확인한다(D-336-E — #366 에서 없어짐: 세션변수 부류도 셋업이 실행 시작 값으로 준다, D-366-06). 보간 함수 피연산자가 아닌 공유 정렬 키는 자기 도메인(D-362-01, 사용자 결정 — develop 은 첫 값 부류) | C | K13·L-21 | — |
| C15 | 비문자 결과 도메인 — `COALESCE(CAST(? AS DATETIME), CAST(? AS DATETIME), ?)`, 산술·날짜 함수 결과, 비문자 게이트 슬롯 | **collation 0 + NORMAL, 플래그 없음**(게이트 표의 비문자 항목에 collation 칸 없음). `pt_upd_domain_info` 가 인자 플래그를 비문자 결과로 옮기는 경로는 이 PR 결함 수정 | C | K15·U13·L-19 | 결함 소멸(optdebug assert → 값) — **#358**: develop 재현 없음(해당 없음); 비문자 결과의 답 차이(NULL 상수 피연산자)는 [#364](https://github.com/xmilex-git/workspace/issues/364) 가 고쳤다(develop 답) |
...
| **세션변수 문장 내 타입 변경 → 실행 전 오류**(#366, 사용자 결정 U3 — 위 행과 D-336-E 대체) | S4·S5 | 문장이 읽는 변수에 CAST 없이 다른 타입을 대입하면 행 전에 -1384 `ER_QPROC_SESSION_VARIABLE_TYPE`. develop 은 행마다 값 타입으로 계산했다(`set @a = 0; select a, @a := (@a + (a + a) / (a * 2)) * (c * 10) + 1 from …` float c: develop 5.0·19.0·41.0·43.0 → -1384; `set @v = 1; select @v := @v + 1, @v := '2.5' …` → -1384). 문자열의 길이·CHAR/VARCHAR 차이는 같은 타입이라 답 불변(`set @b = ''; select @b := @b + cast(a as varchar(100)) + b …`). 내용 부류는 시작 값의 것(D-366-06): 파생 테이블이 행 전에 `@m := '01:00:00'` 으로 바꾼 `median(@m)` 은 시작 `'1.5'` 의 DOUBLE 부류로 -181(develop TIME), STR_TO_DATE 포맷을 바꾸면 시작 포맷의 DATE(develop DATETIME) | P7 ①③ | upstream `bug_bts_6605` 1문장, 캠페인 TC `fetch_gate_probes` [S5]·`fetch_gate_counters`·`list_aggregate_probes` [CLASS]·`analytic_interpolation_keys` [SESSION_CHANGED], 새 TC `session_variable_types` ([#366 해소](https://github.com/xmilex-git/workspace/issues/366)) |
