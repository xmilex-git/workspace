...
| U13 | `COALESCE(CAST(? AS DATETIME), CAST(? AS DATETIME), ?)` | 형제 DATETIME 미러; 비문자 결과 도메인에 collation 플래그 금지 → **게이트 확정(값 도메인, develop 격자) — D-336-B(2026-09-24)** | G | K 게이트 | 현행 값; optdebug assert(L-19) 소멸 | 결함 소멸 — **철회(D-336-A)**: 답 불변(develop) — **#358**: develop 에 L-19 결함이 없다(csql·JDBC, 두 빌드). dpin 은 NULL 인 상수 피연산자를 계획 도메인으로 접어 답이 develop 과 다르다 → [dpin-17g #364](https://github.com/xmilex-git/workspace/issues/364) |
