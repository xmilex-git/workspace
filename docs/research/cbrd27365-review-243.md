...
- **수정 후(optdebug)**: smoke ×2 csql PASS / scroll PASS. csql 형상 6종(2분기 10,000 · 3분기 15,000 · 공집합 분기 5,000 · distinct 5,000 · ORDER BY LIMIT · 서브쿼리 혼합 200) 정확, 서버 .err assert/ERROR 0. JDBC `UnionScroll`(scroll-insensitive, fetch 100, 5,000행/분기, shape 별 별도 커넥션) **수정 전/후 A/B**(4개 파일만 87433ead2 로 되돌려 incr → 재현 → HEAD 복원 → incr, 심볼 개수 0/2 로 바이너리 확인): 수정 전은 2분기·3분기·공집합 분기 **3 shape 모두** 역방향에서 `Cannot communicate with the broker` + CAS 코어 3개(스택 동일: `cursor_prev_tuple` cursor.c:1583 ← `db_query_seek_tuple` db_query.c:2708); 수정 후는 2분기 10,000/10,000 합 25,005,000 · 3분기 15,000/15,000 합 37,507,500 · 공집합 5,000/5,000 합 12,502,500 모두 fwd=bwd, 예외·코어 없음(`A-before-unionscroll*.{out,txt}`, `C-after-unionscroll.out`). 기존 smoke(30행)는 수정 전/후 모두 PASS — UNION 경로를 타지 않으므로 이 결함을 못 잡는 것이 확인됨(D-243-5 의 근거).
