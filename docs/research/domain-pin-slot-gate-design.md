...
| **D-336-E** | ~~세션변수: 게이트가 실행 시작 시 저장값 타입으로 노드 도메인을 정하되, **행의 값 타입이 그 결정과 다르면 develop 처럼 값 타입으로 늦은 해석**한다(오류·변환 없음, P-S2 `(@v := @v + 1), (@v := '2.5')` 는 develop 의 4). 표 재조회(D-325-10)는 dpin-14 몫.~~ **대체(2026-09-27, [#366](https://github.com/xmilex-git/workspace/issues/366), 사용자 결정 U1~U4)**: 문장이 읽는 변수의 타입은 게이트가 문장 동안 하나로 정하고(시작 값 타입 + 대입식 타입), 다른 타입은 실행 전 -1384 다 — P-S2 는 -1384. | develop 동일 | 종료 조건 2 의 "세션변수 셀 fetch 카운터 0" 은 타입이 바뀌지 않는 문장에서만 0 | dpin-14 가 재조회로 바꿀 때 |
