...
로드(서버)           domain_plan: (가) 는 고정 항목, (나) 는 슬롯 항목, 게이트 의존 노드는 gate_nodes 목록.
                     경계 (a): 그 밖에 VARIABLE 이 남으면 -1382 (예외 X-1~X-5·X-8~X-11 만)
...
fetch                게이트 의존 노드는 G1 의 결정을 읽는다(문자 결과·MySQL 호환 모드·세션변수 타입 변경 행은
                     develop 의 늦은 해석 유지 — #338·dpin-14; CAST 노드는 컴파일 대상 도메인 그대로 —
...
- 문자 세션변수(`@b` CHAR(8) 이 concat 으로 자라는 bug_bts_4562)는 문자 결정을 읽지 않는 규칙(#338)으로 자동으로 늦은 해석.
...
| 티켓 | 영향 |
|---|---|
| #337 dpin-11 | 없음(파생 소비자 X-8~X-10 그대로). |
| **#338 dpin-12** | 같은 원칙의 검토 필요: "LEAVE 0·직접 인자 컴파일 ENFORCE" 는 컴파일이 collation 을 정하는 것이라 답(collation·-1150/-622 시점)이 develop 과 갈릴 수 있다. 이 문서의 원칙대로면 collation 도 게이트가 값에서 정하고 답은 develop — #338 본문을 그 방향으로 고칠지 결정이 필요(열린 결정 Q2). |
