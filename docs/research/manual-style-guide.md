...
- **한다체.** 모든 본문 문장은 `-다`로 끝난다. 합니다체·명령형(~하십시오) 금지.
  ...
  - 권장: "…하는 것을 권장한다."
- **"질의"만 사용한다 — "쿼리"는 금지** (#723 리뷰어 교정, 전역 적용됨).
...
- **단어를 쓰기 전에 매뉴얼에 이미 있는 동의어를 확인하고, 있으면 그 표현을 그대로 쓴다 (집필 티켓 #152, 사용자 교정 4건째).** 신조어가 아닌 평범한 단어라도 매뉴얼이 같은 개념을 다른 표현으로 이미 부르고 있으면 그쪽이 규범이다. 실제 사례: "병합"이라고 썼지만 매뉴얼의 View Merging 절(:ref:`view_merge`)은 기능명을 **View Merging** 영문 그대로, 동사는 "머지(merge)"로 쓴다 — 표현을 절 원문과 일치시키고 상호 참조를 걸었다. 절차: 개념을 지칭하는 기존 절이 있는지 grep → 그 절의 명사/동사 표현을 그대로 차용 → 가능하면 :ref: 상호 참조.


- **구어체 엔진 은어를 프로즈에 쓰지 않는다 (PR #768 리뷰어 shparkcubrid 교정, 티켓 #154).** "디스크에 **떨어진**" 같은 개발자 구어(spill의 구어 번역)는 매뉴얼 문체가 아니다. ① 디스크 상주 여부가 그 문장에서 비본질이면 통째로 삭제한다(리뷰어 제안: "부질의 등으로 생성된 임시 결과 리스트"). ② 의미가 본질(예: 제약 조건 "임시 파일로 저장되지 않은 경우")이면 "**저장된/저장되지 않은**"으로 쓴다. ko를 고치면 en도 대칭 확인 — en의 "spilled to disk"는 표준 용어라 그 자체는 무방하지만, ko가 디스크 언급을 삭제한 문장은 en도 함께 삭제한다. 일반화: 사내/개발자 구어가 의심되면 매뉴얼 전체를 grep해 선례 0건이면 쓰지 않는다(§12 신조어 규범과 동일 절차).

- **기준 지표가 다른 규칙은 '동일한 규칙'이 아니라 '같은 형태의 규칙'으로 상호 참조한다 (PR #768 리뷰 2라운드, 티켓 #154).** 규칙의 모양(활성화 조건·증가 방식·상한)이 같아도 입력 지표(스캔 페이지 수 vs 큰 쪽 리스트 페이지 수)가 다르면 '동일'은 부정확하다. en도 "the same rule" 대신 "a rule of the same shape"로 대칭.

- **비적용·제약 조건을 "~와 같은 일부 경우"로 뭉뚱그리지 않는다 (PR #768 리뷰 2라운드).** 엔진의 자격 검사 함수(예: memoize `possible_check`, `px_scan_checker`)에서 조건을 전수 도출해 불릿으로 나열한다. 단 '전수'는 **사용자가 SQL로 표현·관찰할 수 있는 조건의 전수**로 한정 — 내부 플랜 형태(클래스 속성 조회 등)까지 나열하면 오히려 무의미하다. 나열 전 각 항목을 코드로 검증한다(리뷰어 목록도 검증 후 수용).

- **파라미터 표의 셀을 빈칸으로 두지 않는다 (PR #768 리뷰 2라운드).** 상한은 코드(`system_parameter.c`의 prm 정의)에서 검증해 실존 값을 쓰고, 상한이 없으면(upper=NULL) ko '없음'/en 'none'을 명기한다. 코드에 없는 상한(INT_MAX 등)을 관례라는 이유로 지어내지 않는다.

- **사용자가 관찰 가능한 임계 수치는 히든 상수라도 프로즈에 문서화한다 (PR #768 리뷰 2라운드).** `MIN_TUPLES_FOR_PARALLEL_SORT=500` 같은 소스 상수도 동작 경계(500행 미만 직렬)가 사용자에게 보이면 수치로 쓴다. 이는 "히든 파라미터는 config.rst에 항목 비추가" 결정과 별개다(파라미터 문서화 ≠ 동작 수치 문서화). 단 상수의 **적용 경로를 코드로 확인**해 범위를 정확히 한정한다(500은 ORDER BY+LIMIT 전용이었고 일반 정렬 게이트는 별개 규칙).

- **다단 클램프·조정은 적용 순서까지 명시한다 (PR #768 리뷰 2라운드).** "크면 낮추어 적용된다" 한 문장으로 합치지 말고, 부팅 시 조정처럼 순서가 있는 경우(①코어 수 ②max_parallel_workers) 순서 있는 불릿로 쓴다.

- **RST 이스케이프(`\ `) 편집 뒤 조사 중복을 기계 스윕한다 (PR #768 리뷰 2라운드).** ":ref:`...`\에 결과에"처럼 이스케이프 조사와 뒤따르는 명사구 조사가 중복되는 오타가 생기기 쉽다. en QA 3패스의 기계 스윕에 ko 항목으로 추가: `` `\<조사> `` 직후 어절의 조사 중복 여부를 grep으로 점검한다.

- **trace 관찰 서술은 "출력 조건"까지 코드로 확인한다 (PR #768 리뷰 2라운드, MEMOIZE 교훈).** 필드 값 변화(enabled: false)로 서술하기 전에 그 상태가 완성된 trace에서 실제로 관찰 가능한지 확인한다 — MEMOIZE는 캐시 해제 시 storage가 해제되어 **항목 자체가 출력되지 않으므로** enabled: false는 관찰 불가능한 상태였다. "hit>0일 때만 출력" 같은 출력 게이트가 있는 블록은 게이트 기준으로 서술한다.
