...
4. **미디어 복구는 완결된 빌드의 차단벽을 넘지 않는다.** restoredb 계열의 redo가 이 레코드를
   만났을 때, 그 빌드 트랜잭션이 replay 창 안에서 완결(commit/abort)되는 경우에만 전용
   에러(영/한 메시지)로 즉시 실패한다: "이 백업 체인은 loaddb의 no-redo 인덱스 빌드 구간을
   포함하여 재생 복원할 수 없습니다. loaddb 이후 백업을 사용하거나 시점 복원으로 그 이전까지만
   복원하십시오." 트랜잭션이 창 안에서 완결되지 않으면(-d 시점복원이 완결 전에서 멈추는 경우,
   crash로 끊긴 빌드) undo가 인덱스 파일째 제거해 "그 문장 미실행 상태"가 되므로 차단벽을
   no-op으로 통과한다 — 시간 기반 stop은 완결 레코드에서만 멈출 수 있어 redo가 차단벽을
   물리적으로 지나가기 때문에, 조우가 아니라 완결이 거부 기준이다. 재기동(crash recovery)은
   차단하지 않는다(커밋 전 flush 관문으로 페이지가 이미 디스크에 있어 무해 통과). 시점
   복원(-d, 백업 자체 복원)은 정상 동작. 결과: 불완전 인덱스가 존재하는 DB 상태는 어떤
   경로로도 생성 불가.
