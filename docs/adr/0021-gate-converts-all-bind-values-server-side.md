...
- NULL 원 값은 변환기 밖에서 목표 도메인의 typed NULL 로(D-325-08). 휘발 피연산자(세션변수)의 값은 행마다 읽되 도메인·실패 정책은 게이트 1회, 행 값 타입이 바뀌면 그 행에서만 표 재조회, 비교 문맥 실패는 -494(D-325-10). — **정정 4(2026-09-27, #366)**: 세션변수는 문장 타입을 게이트가 1회 정하고(실행 시작 값의 타입 + 대입식의 계획 도메인), 다른 타입은 실행 전 -1384 다. 행 값 타입이 바뀌어 표를 다시 찾는 일은 없다(ADR 0020 의 #366 구현 주석).
