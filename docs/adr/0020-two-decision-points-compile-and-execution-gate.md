...
- **구현 주석(2026-09-26, #355)**: 플랜 불변이 끝났다. develop 이 실행 중 노드에 쓰던 답(D-336-E 행 바인딩, 문자 LEAVE 노드의 첫 계산, 셋업이 주는 집계·분석·위치·출력 열 도메인과 연산자 타입)은 XASL_STATE 의 노드 칸(`RESOLVED_DOMAIN_TABLE.taken`·`taken_list`·`taken_type`)에 있다 — 게이트 표의 봉인된 결정과 달리 실행의 주인 스레드가 쓰는 실행별 기록이고 새 결정 지점이 아니다. 칸은 노드마다 따로이고(값 포인터가 생산자 항목을 공유해도, D-355-09) 실행 동안 남아 develop 의 쓰기·원복 이력과 같다. 정렬 키는 실행 소유 사본, FAST_PEEK 은 적재, 비교 기록은 항목에 있다. 위 첫 결과의 원복·원본 저장·`original_*` 필드는 이제 없고 regu/arith/pos_descr/agg/analytic 크기는 develop 과 같다. 결정 원문: [#355 해소](https://github.com/xmilex-git/workspace/issues/355).
