...
> **사용자 검토 반영본 · 2026-09-05.** 본문은 구조 변경을 논의하기 위한 설명이다. 주요 코드를 리뷰하려면 [코드 해설](develop-vs-cas-merge-code-walkthrough.md), 운영 정책·검증 수치·전체 변경 파일은 [부록](develop-vs-cas-merge-appendices.md)을 읽는다.
>
...
**이 통합의 중심은 SQL 컴파일·권한 처리·PL 내부 SQL이 프로세스 경계를 오가던 구조를 정리하는 것이다.** CAS에서 하던 client 절반과 드라이버 요청 처리를 `cub_server`로 옮겨, 컴파일러와 실행기가 같은 서버 세션 문맥에서 함수로 호출되게 한다. 브로커는 최초 접속을 받아 대상 DB 서버에 연결을 넘기는 프런트로 남는다.

성능 실측은 이 구조 변경의 효과를 판단하는 근거다. 기존 단문 비교의 hop 추가 비용 약 31µs와 재컴파일 추가 비용 약 380µs는 별개이며, 통합이 후자를 전부 없앤 것은 아니다. 기존 YCSB에서는 처리량이 C +12.5%·A +6.5% 개선됐지만 READ p99 악화도 관측됐다. 측정 커밋과 조건은 검증 절에서 구분한다.

동시에 **CAS 프로세스가 제공하던 장애 격리와 풀 다중화를 잃는다.** 컴파일러 메모리 손상은 서버 전체 장애로 확대될 수 있고, 현재 연결당 전담 스레드와 세션 상태의 비용을 서버가 부담한다. 이 대가까지 포함해 구조 변경의 타당성을 논의해야 한다.
